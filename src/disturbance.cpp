...
float Disturbance::getAngle (b2Transform t)
{ //gets the angle of an Disturbance wrt to another Disturbance (robot)
    ...
    float cosA = (thisToB.x * cos (t.q.GetAngle ())
                  + thisToB.y * sin (t.q.GetAngle ()))
                 / thisToB.Length ();
    angle = acos (cosA);
    return angle;
}
...
{
    bool _position = std::round (bf.pose.p.x * 100) / 100
                         == std::round (d.bf.pose.p.x * 100) / 100
                     && std::round (bf.pose.p.y * 100) / 100
                            == std::round (d.bf.pose.p.y * 100) / 100;
    bool _pose
        = _position && (bf.pose.q.GetAngle () == d.bf.pose.q.GetAngle ());
    bool dim = (bf.halfLength == d.bf.halfLength)
               && (bf.halfWidth == d.bf.halfWidth);
    bool aff = affordanceIndex == d.affordanceIndex;
    return _pose && dim && aff;
}

std::vector<b2Vec2> GetLocalPoints (std::vector<b2Vec2> pts,
                                    const b2Body *body)
{
    std::vector<b2Vec2> result;
    for (b2Vec2 p : pts)
