...
FocusedConfigurator::add_vertex_now (const vertexDescriptor &src,
                                     vertexDescriptor &v1, Disturbance Di,
                                     Edge edge, bool topDown)
{
    std::pair<edgeDescriptor, bool> result
        = addVertex (src, v1, edge, topDown);
...
std::pair<edgeDescriptor, bool> FocusedConfigurator::add_vertex_retro (
    vertexDescriptor &src, vertexDescriptor &v1, Edge edge, bool topDown)
{
    std::pair<edgeDescriptor, bool> result
        = addVertex (src, v1, edge, topDown);
...
Disturbance FocusedConfigurator::getDisturbance (TransitionSystem &g,
                                                 vertexDescriptor v,
                                                 b2World &world,
                                                 const Direction &dir,
    ...
    if (!g[v].Dn.isValid ())
    {
        ...
        if (visited.first || out.empty ())
        {
            ...
            { //if Di is valid and not the same direction as the vertex
                ...
                b2AABB box = worldBuilder->makeRobotSensor (
                    robot.body (), controlGoal.get_disturbance ());
...
FocusedConfigurator::explorer (vertexDescriptor v, TransitionSystem &g,
                               b2World &w)
{
    ...
    do
    {
        ...
        er = controlGoal.checkEnded (g[v], t.get_direction (),
                                     true); //check ended with relax
        applyTransitionMatrix (v, direction, er.ended, v, plan_prov);
        EvaluationQueueManager eqm;
        for (Direction d : g[v].options)
        { //add and evaluate all vertices
            v0_exp = v;
            ...
            while (!options.empty ())
            {
                ...
                do
                {
                    std::pair<State, Edge> sk = simulation_setup (
                        w, t, v0, shift, start, g[v0].options);
                    simResult sim = simulate (t, w); //sk.first, g[v0],
                    gt::fill (sim, &sk.first,
                              &sk.second); //find simulation result
                    sk.second.it_observed = iteration;
                    er = estimateCost (sk.first, g[v0].endPose,
                                       sk.first.direction, controlGoal);
                    StateDifference sd;
                    VertexMatch match
                        = findMatch (sk.first, t.get_direction (),
                                     desiredMatch (), &sd); //, closest_match
                    std::pair<edgeDescriptor, bool> edge (
                        edgeDescriptor (),
                        false); //, new_edge(edgeDescriptor(TransitionSystem::null_vertex(), TransitionSystem::null_vertex(), NULL), false);
                    if (matcher->match_equal (match.first, desiredMatch ()))
                    {
                        g[v0].options.erase (g[v0].options.begin ());
                        edge = setup_match_edge (match, v0, v1, sk.second,
                                                 t.get_direction (), false);
                        if (currentTask.is_over ())
                        {
                            std::vector<vertexDescriptor> task_vs
                                = task_vertices (v1);
                            vertexDescriptor task_start = task_vs[0];
                            startRecycle
                                = getRecyclingStart (v, v1, task_start);
                            if (plan_prov.empty ())
                            {
                                recycle_plan (startRecycle, v0, task_start,
                                              match.first, shift_start,
                                              sk.first.start, edge, plan_prov,
                                              t.get_direction ());
                            }
                            if (hasPlanFinished ()
                                && g[task_start].options.empty ()
                                && g[v].options.empty ())
                            {
                                if (startRecycle != v)
                                {
                    ...
                    applyTransitionMatrix (v1, t.get_direction (), er.ended,
                                           v0, plan_prov);
                    g[v1].phi = evaluationFunction (er, v1, plan_prov);
                    propagateD (
                        v1,
                        v0); //if v0 is a dummy vertex it propagates the disturbance
                    v0_exp = v0;
                    options = g[v0_exp].options;
                    v0 = v1;
                    eqm.addToEvaluationQueue (evaluationQueue, v1,
                                              transitionSystem, v);
                }
                while (t.get_direction () != DEFAULT
                       & int (g[v0].options.size ()) != 0);
            }
        }
        backtrack (evaluationQueue, priorityQueue, closed, plan_prov, v,
                   startRecycle);
...
std::vector<vertexDescriptor>
FocusedConfigurator::splitTask (vertexDescriptor v, Direction d,
                                vertexDescriptor src)
{
    ...
    { //d
        if ((src == MOVING_VERTEX || src == currentVertex)
            && transitionSystem[v].outcome == simResult::crashed)
        {
    ...
    auto ie = inEdges (src);
    auto sameIterationEdgeIt
        = check_vector_for (ie, SameIteration (transitionSystem, iteration));
    if (!transitionSystem[src].isTurning ()
        && (!ie.empty () || src == MOVING_VERTEX || src == currentVertex))
    { //! //&& sameIterationEdgeIt!=ie.end()
        transitionSystem[src].outcome = simResult::safeForNow;
        split.insert (split.begin (), src);
    }
    vertexDescriptor v1 = v;
    float nNodes = transitionSystem[v].distance () / _customStep,
          og_phi = transitionSystem[v].phi;
    ...
    while (nNodes > 1)
    {
        ...
        if (nNodes > 1)
        {
            ...
            {
                first_edge = addEdgeRetrospectively (v, v1, s_tmp, first_edge,
                                                     d, a.getLinearSpeed ());
                split.push_back (
                    v1); //insert at the end if state has to be created
            }
            else
            {
                v1 = match.second;
        ...
        if (nNodes <= 1)
        {
            ...
            if (match.first != StateMatcher::_TRUE || match.second == v)
            {
                if (v1 == *(split.end () - 1))
                {
                    ...
                    transitionSystem[first_edge.first].step
                        = gt::distanceToSimStep (
                            transitionSystem[v1].distance (),
                            a.getLinearSpeed ());
                }
...
void FocusedConfigurator::backtrack (
    std::vector<vertexDescriptor> &evaluation_q,
    std::vector<vertexDescriptor> &priority_q,
    std::set<vertexDescriptor> &closed,
    std::vector<vertexDescriptor> &plan_prov, vertexDescriptor module_src,
    vertexDescriptor startRecycle)
{
    for (vertexDescriptor v : evaluation_q)
    {
        ...
        { //
            vertexDescriptor split_v = split[i],
                             src = TransitionSystem::null_vertex ();
            if (i < 1)
            {
                auto likelyEdge = gt::getMostLikely (
                    transitionSystem, inEdges (split_v), iteration);
                if (likelyEdge.first)
                {
            ...
            EndedResult local_er = estimateCost (
                transitionSystem[split_v], transitionSystem[split_v].start,
                direction, controlGoal);
            transitionSystem[split_v].phi
                = evaluationFunction (local_er, split_v, plan_prov);
            applyTransitionMatrix (split_v, direction, local_er.ended, src,
                                   plan_prov);
...
bool FocusedConfigurator::propagateD (vertexDescriptor v1, vertexDescriptor v0)
{
    ...
    if (isTurning (transitionSystem[v1].direction)
        != isTurning (transitionSystem[v0].direction))
    {
        return false;
    }
    bool same_Di = transitionSystem[v0].Di == transitionSystem[v1].Di;
    if (((canPropagate (v0) && same_Di
          && (transitionSystem[v0].Dn.getAffIndex () == NONE
              || !transitionSystem[v0].filled))))
    {
...
void FocusedConfigurator::visitedDirectionsPushBack (
    vertexDescriptor v, std::vector<Direction> &visitedDirections,
    Direction direction)
{
    for (edgeDescriptor &e : gt::outEdges (transitionSystem, v, direction))
    {
        if (transitionSystem[e].it_observed == iteration)
        { //g[e.m_target].visited()
            visitedDirections.push_back (
                transitionSystem[e.m_target].direction);
        }
    }
}

std::vector<Direction> FocusedConfigurator::getExploredDirections (
    vertexDescriptor v, const std::vector<Direction> &directions)
{
...
void FocusedConfigurator::removeExploredTransitions (vertexDescriptor v)
{
    std::vector<Direction> options
        = getExploredDirections (v, transitionSystem[v].options);
...
{
    if (round (transitionSystem[v].endPose.p.Length () * 100) / 100
        >= BOX2DRANGE)
    {
        if (transitionSystem[v].isTurning ())
        {
        ...
        if (auto def_it
            = std::find (transitionSystem[v].options.begin (),
                         transitionSystem[v].options.end (), DEFAULT);
            def_it != transitionSystem[v].options.end ())
        {
...
void FocusedConfigurator::transitionMatrix (vertexDescriptor v, Direction d,
                                            vertexDescriptor src)
{
    Task temp (controlGoal.get_disturbance (), DEFAULT,
               transitionSystem[v].endPose); //reflex to disturbance
    srand (unsigned (time (NULL)));
    auto oe = gt::outEdges (transitionSystem, v, d);
    std::pair<bool, edgeDescriptor> ve
        = gt::visitedEdge (oe, transitionSystem, currentVertex);
    if (shouldPartiallyExplore (oe, ve))
    {
        transitionSystem[v].options = partiallyExplorativeOptions (ve);
    }
    else if (transitionSystem[v].outcome == simResult::safeForNow)
    { //accounts for simulation also being safe for now
        if (d == DEFAULT || d == STOP)
        {
            transitionSystem[v].options.push_back (temp.get_direction ());
            transitionSystem[v].options.push_back (
                getOppositeDirection (temp.get_direction ()).second);
            //prioritise reflex
            if (temp.getAction ().getOmega () == 0)
            { //if the task chosen is a turning task
                if (rand () % 2 == 0)
                {
    ...
    else if (transitionSystem[v].outcome == simResult::successful)
    { //will only enter if successful
        if (d == LEFT || d == RIGHT)
        {
            auto defaultVisited
                = gt::visitedEdge (gt::outEdges (transitionSystem, v, DEFAULT),
                                   transitionSystem, currentVertex);
            if (!defaultVisited.first)
            { //used to be just the inside of this statement
                transitionSystem[v].options = { DEFAULT };
                if ((src == currentVertex
                     && controlGoal.getAffIndex () == PURSUE
                     && SignedVectorLength (
                            controlGoal.get_disturbance ().pose ().p)
                            < 0))
                {
                    transitionSystem[v].options.push_back (d);
                }
            }
            else if (transitionSystem[defaultVisited.second.m_target].outcome
                     == simResult::crashed)
            {
                transitionSystem[v].options.push_back (d);
            }
...
void FocusedConfigurator::applyTransitionMatrix (
    ...
    std::vector<vertexDescriptor> &plan_prov)
{
    ...
    else if (round (transitionSystem[v0].endPose.p.Length () * 100) / 100
             >= BOX2DRANGE)
    { // OR g[vd].totDs>4
        return;
    }
    if (src != MOVING_VERTEX
        && uint (src) < (transitionSystem.m_vertices.size () - 1)
        && v0 != MOVING_VERTEX)
    { //src< v size is to check that src isn't a garbage value (was giving throuble with tests)
        auto e = boost::edge (
            src, v0,
            transitionSystem); //not adding options to vertices which don't cover a distance unless they're current v
        if (e.second)
        {
    ...
    full_plan.insert (full_plan.begin (), current_vertices.begin (),
    ...
    //}
    if (auto it = check_vector_for (full_plan, v0);
        it != full_plan.end () && it != (full_plan.end () - 1))
    {
        ...
        {
            if (transitionSystem[e.first.m_target].outcome
                == simResult::crashed)
            {
                transitionMatrix (v0, d, src);
                erase_from_vector (
                    transitionSystem[v0].options,
                    transitionSystem[e.first.m_target].direction);
            }
...
void FocusedConfigurator::addToPriorityQueue (
    vertexDescriptor v, std::vector<vertexDescriptor> &queue,
...
vertexDescriptor
FocusedConfigurator::getNextSrc (const std::vector<vertexDescriptor> &q)
{
    int index = 0;
...
VertexMatch
FocusedConfigurator::hardMatch (State s, Direction dir,
                                StateMatcher::MATCH_TYPE match_type,
                                StateDifference *_sd)
{
    VertexMatch result (StateMatcher::MATCH_TYPE::_FALSE,
                        TransitionSystem::null_vertex ()),
        backup = result;
    auto vs = boost::vertices (transitionSystem);
    float prob = 0, sum = 10000;
    typedef std::tuple<vertexDescriptor, StateMatcher::MATCH_TYPE, float>
        VertexMatchTuple;
    ...
    for (auto vi = vs.first; vi != vs.second; vi++)
    {
        ...
        bool Tmatch = dir == Direction::UNDEFINED
                      || transitionSystem[v].direction == dir
                      || (transitionSystem[v].direction == STOP
                          && dir == DEFAULT && iteration > 1);
        ...
        if (check_vector_for (vertices, currentVertex) != vertices.end ()
            && !currentTask.is_over ())
        { //v==currentVertex
        ...
        try
        {
            m = matcher->isMatch (sd, tracker->get_threshold (q),
                                  s.endPose.p.Length ());
        }
        catch (std::exception &e)
        {
            std::cerr << "check tracker is set up ok! " << e.what ()
                      << std::endl;
        }
        condition = matcher->match_equal (m, match_type);
        if (v != MOVING_VERTEX
            && (boost::in_degree (v, transitionSystem) > 0 || iteration > 1)
            && Tmatch)
        {
            // if (condition){
            // 	result.first= m;
...
VertexMatch
FocusedConfigurator::findMatch (State s, Direction dir,
                                StateMatcher::MATCH_TYPE match_type,
                                StateDifference *_sd, vertexDescriptor src)
{
    if (s.start == b2Transform_zero
        && s.direction == currentTask.get_direction ()
        && //s.Di==transitionSystem[currentVertex].Di &&
        !hasPlanFinished ()
        && s.Dn.getAffIndex ()
               == transitionSystem[currentVertex].Dn.getAffIndex ())
    { //if the state to be matched is the current one, return it
        return VertexMatch (StateMatcher::_TRUE, currentVertex);
    }
    return hardMatch (s, dir, match_type, _sd);
}

void FocusedConfigurator::planPriority (TransitionSystem &g,
                                        vertexDescriptor v)
{
...
{
    FilteredTS fts (transitionSystem, ViableEdge (&transitionSystem),
                    Connected (&transitionSystem)); //boost::keep_all()
...
void FocusedConfigurator::shift_states (TransitionSystem &g,
                                        const std::vector<vertexDescriptor> &p,
                                        const b2Transform &shift_start)
{
    if (p.empty ())
    {
        return;
...
void FocusedConfigurator::pre_explore ()
{
    ...
    {
        boost::remove_out_edge_if (MOVING_VERTEX, is_not_v (currentVertex),
                                   transitionSystem);
    }
...
void FocusedConfigurator::explore_plan (b2World &world)
{
    ...
    resetPhi ();
    transitionSystem[MOVING_VERTEX].phi
        = evaluationFunction (EndedResult (), MOVING_VERTEX, m_plan);
    std::vector<vertexDescriptor> plan_tmp
        = explorer (src, transitionSystem, world);
    if (DEBUG)
    {
        std::vector<vertexDescriptor> _plan = (m_plan);
        debug::graph_file (iteration, transitionSystem,
                           controlGoal.get_disturbance (), _plan,
                           currentVertex);
    }
    ts_cleanup (); //remove self-edge and singleton states
    if (plan_tmp.empty ()
        && (!transitionSystem[currentVertex].visited ()
            || currentTask.is_over ()))
    { //currentv not visited means that it wasn't observed ()
        printf ("no plan, searchign from %li\n", src);
        printf ("src =%li out degree=%li\n", src,
                boost::out_degree (src, transitionSystem));
        bool finished = false;
        ExecutionInfo info = package_info ();
        plan_tmp
            = planner->plan (transitionSystem, src, info, &finished); //src
    }
    m_plan = plan_tmp;
    //enforce_edge();
    //printPlan(&m_plan);
...
std::pair<State, Edge> FocusedConfigurator::simulation_setup (
    b2World &w, Task &t, vertexDescriptor v0, b2Transform shift,
    b2Transform &start, std::vector<Direction> v0_options)
{
    start = b2Mul (shift, transitionSystem[v0].endPose);
    Disturbance Di
        = getDisturbance (transitionSystem, v0, w, v0_options[0], start);
    ...
    worldBuilder->buildWorld (w, t.getStart (), t.get_direction (),
                              t.get_disturbance (), 0.15,
                              WorldBuilder::PARTITION); //was g[v].endPose
    return sk;
}

void FocusedConfigurator::reassign_direction (vertexDescriptor bestNext,
                                              Direction &direction)
{
...
std::pair<edgeDescriptor, bool> FocusedConfigurator::setup_match_edge (
    VertexMatch &match, vertexDescriptor &v0, vertexDescriptor &v1,
    const Edge &k, Direction direction, bool changedMatch)
{
    v1 = match.second; //frontier
    std::pair<edgeDescriptor, bool> edge = gt::add_edge (
        v0, v1, transitionSystem, iteration, direction); //assumes edge added
    if (edge.second)
    {
...
std::vector<vertexDescriptor>
FocusedConfigurator::task_vertices (vertexDescriptor v,
                                    std::pair<bool, edgeDescriptor> *ep)
{
    ...
    do
    {
        ...
        if (ep2.first)
        {
            ...
            {              //size 1
                ...
                {
                    if (transitionSystem[e.m_target].direction == d
                        && //e!=ep2.second &&
                        ...
                        transitionSystem[e.m_source].Di.bf.match (
                            transitionSystem[_ep.second.m_source].Di.bf)
                        && transitionSystem[e.m_source].Dn.bf.match (
                            transitionSystem[_ep.second.m_target].Dn.bf))
                    {
                        ep2.second = e;
                        break;
                    }
                     ...
                     && transitionSystem[ep2.second.m_target].Di
                            == transitionSystem[_ep.second.m_target].Di
                     && transitionSystem[ep2.second.m_target].Dn
                            == transitionSystem[_ep.second.m_target].Dn)
            {                                           //same task!
        ...
        if (ep2.second.m_target == currentVertex)
        { //source
            break;
        }
    }
...
bool FocusedConfigurator::isPreviousState (const State &s,
                                           const State &candidate)
{
    StateMatcher::MATCH_TYPE match
        = matcher->isMatch (s, candidate, tracker->get_threshold (s));
    if (!(matcher->match_equal (match, StateMatcher::DI_SHAPE)
          && matcher->match_equal (match, StateMatcher::DN_SHAPE)))
    {
        return false;
    }
    if (!(s.Di.getAffIndex () == candidate.Di.getAffIndex ()
    ...
    bool Dn = false,
         Di
         = false; //likey to be previous state in the same task based on Dn/Di
    ...
    {
        Di = s.end_from_Di ().p.Length ()
             <= candidate.end_from_Di ().p.Length ();
    }
    else
    {
        Di = s.end_from_Di ().p.Length ()
...
std::vector<edgeDescriptor> FocusedConfigurator::inEdges (vertexDescriptor v,
                                                          Direction d)
{
...
vertexDescriptor FocusedConfigurator::getRecyclingStart (
    vertexDescriptor v, vertexDescriptor v1, vertexDescriptor taskStart)
{
...
bool FocusedConfigurator::closeVertex (std::set<vertexDescriptor> &closed,
                                       vertexDescriptor v)
{
    int MAX_OUT = 3;
    if (transitionSystem[v].isTurning ())
...
std::pair<edgeDescriptor, bool> FocusedConfigurator::addEdgeRetrospectively (
    vertexDescriptor v, vertexDescriptor &v1, const State &s_tmp,
    std::pair<edgeDescriptor, bool> first_edge, Direction d, float linearSpeed)
{
    transitionSystem[v].options = { d };
    transitionSystem[v].endPose = s_tmp.endPose;
    transitionSystem[v].Dn = s_tmp.Dn;
    transitionSystem[first_edge.first].step
        = gt::distanceToSimStep (transitionSystem[v].distance (), linearSpeed);
...
bool FocusedConfigurator::correctQueue (std::vector<vertexDescriptor> &queue,
                                        vertexDescriptor v,
                                        vertexDescriptor startRecycle,
                                        int planProvSize)
{
...
void FocusedConfigurator::adjustProbability (const edgeDescriptor &e)
{
    ...
    std::vector<edgeDescriptor> es = gt::outEdges (
        transitionSystem, e.m_source, transitionSystem[e.m_target].direction);
    float totObs = 0;
    //find total observations
    for (edgeDescriptor &ei : es)
    {
        transitionSystem[ei].probability
            = transitionSystem[ei.m_target].nObs / (float)es.size ();
    ...
    {
        transitionSystem[ei].probability
            = transitionSystem[ei.m_target].nObs / totObs;
    }
}

void FocusedConfigurator::abandonPlan (std::vector<vertexDescriptor> &planProv,
                                       vertexDescriptor v0,
                                       vertexDescriptor v1)
{
    if (current_vertices[0] != DUMMY)
    { //dummy
...
std::vector<Direction> FocusedConfigurator::partiallyExplorativeOptions (
    std::pair<bool, edgeDescriptor> ve)
{
    std::vector<Direction> result;
    if (ve.first)
    {
        if (transitionSystem[ve.second.m_target].visited ())
        {
            ...
            if (transitionSystem[ve.second.m_target].outcome
                != simResult::crashed)
            {
                return { currentTask.get_direction () };
            }
            //preferred task fails: plan evasion maneuvre
            else if (transitionSystem[ve.second.m_target].outcome
                     == simResult::crashed)
            {
...
Robot FocusedConfigurator::makeRobot (b2World &world, const Task &task)
{
    ...
    b2AABB sensor_aabb = worldBuilder->makeRobotSensor (
        robot.body (),
        task.get_disturbance ()); //Configurator::getGoalDisturbance()
    return robot;
}

void FocusedConfigurator::EvaluationQueueManager::addToEvaluationQueue (
    std::vector<vertexDescriptor> &evaluationQueue, vertexDescriptor v1,
    TransitionSystem &g, vertexDescriptor v)
{
    ...
    if (g[v1].outcome == simResult::successful)
    {
        ...
        {
            if (boost::edge (lastAdded, v1, g).second)
            { //if edge exists
...
bool FocusedConfigurator::shouldPartiallyExplore (
