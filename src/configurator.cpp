...
void Configurator::dummy_vertex (vertexDescriptor src)
{
    ...
    currentTask = Task (controlGoal.disturbance, Direction::STOP,
...
bool Configurator::Spawner ()
{
    ...
    worldBuilder->set_world_objects (worldBuilder->getFeatures (
        data2fp, b2Transform_zero, WorldBuilder::PARTITION));
    auto endTime = std::chrono::high_resolution_clock::now ();
    std::chrono::duration<float, std::milli> d_getFeatures
        = now - endTime; //in seconds
    float duration_getFeatures
        = abs (float (d_getFeatures.count ()) / 1000); //express in seconds
    explore_plan (world);
    endTime = std::chrono::high_resolution_clock::now ();
    std::chrono::duration<float, std::milli> d_withExplore
        = now - endTime; //in seconds
    float duration_withExplore
        = abs (float (d_withExplore.count ()) / 1000); //express in seconds
    //FORMAT: vertices	bodies tasks	total_dur	just_worldbuilding
    if (logger)
    {
        logger->log ("%i\t%i\t%i\t%0.6f\t%0.6f\n",
                     transitionSystem.m_vertices.size (), worldBuilder->bodies,
                     simulatedTasks, duration_withExplore,
                     duration_getFeatures);
    }
...
simResult Configurator::simulate (Task t, b2World &w)
{ //State& state, State src,
    ...
    result.endPose.q.Set (approximate_angle (result.endPose.q.GetAngle (),
                                             t.direction, result.resultCode));
...
Configurator::addVertex (const vertexDescriptor &src, vertexDescriptor &v1,
...
{ //returns edge added
    ...
    if (transitionSystem[src].options.size () > 0 || topDown)
    {
        ...
        if (!topDown)
        {
            transitionSystem[src].options.erase (
                transitionSystem[src].options.begin ());
        }
    }
    return result;
}
...
void Configurator::printPlan (std::vector<vertexDescriptor> *p)
{
    ...
    for (vertexDescriptor v : _plan)
    {
        ...
        printf ("%li, %s; ", v,
                (*a).second); //, transitionSystem[edge.first].step
    }
    printf ("\n");
}
...
void Configurator::newScanEvent ()
{
    ...
    if (getIteration () > 1)
    {
        TrackingResult trackingResult (
            currentTask
                .get_disturbance ()); //default tracking returns a zero transform and the current disturbance
    ...
    tracker->on_new_reading (
        currentTask, controlGoal); //do something at every LiDAR reading
    if (goal_changer != NULL)
    { //not super well implemented maybe
...
float Configurator::approximate_angle (float angle, Direction d,
...
void Configurator::estimate_current_vertex ()
{
    ...
    vertexDescriptor task_start = current_vertices[0],
                     cv = TransitionSystem::null_vertex ();
    b2Transform Di_distance = currentTask.from_Di (),
                v_from_D = b2Transform_zero;
    float sum = 10000;
    StateMatcher matcher;
    for (vertexDescriptor &v : current_vertices)
    {
        if ((transitionSystem[task_start].Dn.getAffIndex () == AVOID
             && currentTask.disturbance.getAffIndex () == PURSUE))
        {
        ...
        float sum_diff = fabs (transform_diff.p.x + transform_diff.p.y
                               + transform_diff.q.GetAngle ());
        if (sum_diff < sum)
        {
            cv = v;
            sum = sum_diff;
...
void Configurator::change_task ()
{
    if (!currentTask.is_over ())
    {
        std::cout << "Task change: " << currentTask.get_change ()
                  << "Task step" << currentTask.motorStep << std::endl;
        return;
    }
    ...
    currentTask = task_controller->next_task (
        currentTask, controlGoal, transitionSystem, current_vertices,
        m_plan); //generate next Task following any extracted plan
    std::cout << "new task step = " << currentTask.getMotorStep ()
              << std::endl;
    tracker->on_new_task (
        currentTask,
        controlGoal); //reset tracker to track execution of current task and current goal
    if (control)
    {
        control
            ->reset (); //resets PID controller which right now is not implemented
        control->getData (
            currentTask.action); //actually sends data to the motor controller!
    }
    else
    {
...
void Configurator::update_graph (TransitionSystem &g, const TrackingResult &tr)
{
    ...
    if (DEBUG)
    {
        ...
        fprintf (goalFile, "%0.3f\t%0.3f\n",
                 controlGoal.disturbance.getPosition ().x,
                 controlGoal.disturbance.getPosition ().y);
        fclose (goalFile);
    }
    if (!tracker)
    {
...
{
    if (controlGoal.getAffIndex () == PURSUE && !m_plan.empty ()
        && task_controller->get_disturbance ().getAffIndex () != NONE)
    {
        b2Transform from_Di = b2Transform_zero;
        from_Di = currentTask.from_Di ();
        b2Transform goal_robotPOV = b2Mul (
            from_Di,
            task_controller
                ->disturbance_to_goal ()); //position of goal from the robot based on where it should be from Di
        controlGoal.disturbance.bf.pose = goal_robotPOV;
        debug::print_pose (controlGoal.disturbance.pose (),
                           "goal after adjusting");
...
void Configurator::assignDimensions (Task &t, float halfLength,
                                     float halfWidth)
{
...
void Configurator::adjust_simulated_task (const vertexDescriptor &v, Task &t)
{
    std::pair<edgeDescriptor, bool> ep
        = boost::edge (v, currentVertex, transitionSystem);
    if (!ep.second)
    {           //no tgt
        return; //check until needs to be checked
    }
    if (!t.getEndCriteria ().angle.isValid ())
    {
        return;
    }
    ...
    else if (t.get_direction ()
             == getOppositeDirection (currentTask.get_direction ()).second)
    {
        t.getEndCriteria ().adjust (-tracker->getDeltaTransform ());
    }
}
...
{
    return t.direction == currentTask.direction && t.start == b2Transform_zero
           && t.disturbance.getAffIndex () == currentTask.getAffIndex ();
}
...
void ReactiveConfigurator::explore_plan (b2World &world)
{
    ...
    Task t (currentTask.get_disturbance (), currentTask.get_direction (),
            b2Transform_zero, true);
    worldBuilder->buildWorld (world, b2Transform_zero,
                              currentTask.get_direction ()); //was g[v].endPose
    adjust_simulated_task (currentVertex, t);
    simResult result = simulate (
        t,
        world); //transitionSystem[currentVertex],transitionSystem[currentVertex],
    printf ("crashed=%i, step=%i\n", result.resultCode == simResult::crashed,
            result.step);
    gt::fill (result, &transitionSystem[currentVertex],
              &transitionSystem[currentEdge]);
    //if task crashes, it has to be changed
    currentTask.set_change (transitionSystem[currentVertex].outcome
...
float ReactiveConfigurator::remainingSimulationTime (const Task *const t)
{
    ...
    {
        float r_step = Controller::motor_step (t->getAction (), simulationStep)
                       * MOTOR_CALLBACK;
        printf ("r_step=%f\n", r_step);
        return r_step;
    }
