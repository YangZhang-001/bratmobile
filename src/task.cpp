...
bool overlaps (b2Body *robot, const Disturbance *const disturbance)
{
    ...
    b2Transform robot_pose = robot->GetTransform (),
                d_pose = disturbance->pose ();
    b2PolygonShape d_shape;
    d_shape.SetAsBox (disturbance->bf.halfWidth, disturbance->bf.halfLength,
                      b2Vec2 (0, 0), 0);
    return b2TestOverlap (sensor->GetShape (), 0, &d_shape, 0, robot_pose,
                          d_pose);
}

bool overlaps (const b2PolygonShape &box, const Disturbance *const d,
               const b2Transform &robot_pose)
{
    bool result = true;
...
simResult Task::bumping_that (b2World &_world, int iteration, b2Body *robot,
                              float remaining)
{ //CLOSED LOOP CONTROL, og return simreult
    ...
    for (stepb2d = 0; stepb2d < (HZ * remaining); stepb2d++)
    {
        ...
        if (DEBUG)
        {
            fprintf (robotPath, "%f\t%f\n", robot->GetPosition ().x,
                     robot->GetPosition ().y); //save predictions/
        }
        ...
        if (bool ended
            = checkEnded (robot->GetTransform (), direction, false, robot)
                  .ended;
            ended || out)
        { //out
            //b2Transform expectNextStep=b2Mul(robot->GetTransform(), action.getTransform(1/HZ));
            bool keep_going_out_x
                = (fabs (robot->GetTransform ().p.x + instVelocity.x)
                   > fabs (robot->GetTransform ().p.x))
                  && out_x;
            bool keep_going_out_y
                = (fabs (robot->GetTransform ().p.y + instVelocity.y)
                   > fabs (robot->GetTransform ().p.y))
                  && out_y;
        ...
        { //
            ...
            Disturbance collision
                = Disturbance (listener.get_collisions ()[index]);
...
EndedResult Task::checkEnded (b2Transform robotTransform, Direction dir,
                              bool relax, b2Body *robot,
                              std::pair<bool, b2Transform> use_start)
{ //self-ended , std::pair<bool,b2Transform> use_start
    if (dir == UNDEFINED)
    {
        dir = direction;
    }
    ...
    if (round (distance.Length () * 100) / 100 >= BOX2DRANGE)
    { //if length reached or turn
        r.ended = true;
    }
    if (disturbance.isValid ())
    {
        //this shoudl be replaced by (d orientation == orientation+-PI/2), no robot position at alll
        b2Vec2 v = disturbance.getPosition ()
                   - robotTransform.p; //distance between disturbance and robot
        d = Distance (v.Length ());
        ...
        else if (getAffIndex () == int (InnateAffordances::PURSUE))
        {
            ...
            if (robot != NULL)
            {
                std::vector<b2Vec2> local_vertices
                    = GetLocalPoints (disturbance.vertices (), robot);
                b2Vec2 pos_local
                    = *(std::min_element (local_vertices.begin (),
                                          local_vertices.end (), CompareX ()));
                r.ended
                    = fabs (round (pos_local.x * 100) / 100)
                          <= ((endCriteria.distance.get () - 0.001) / 2)
                      && start.p.x
                             > 0; //hard coded, start x assumes that if the robot has travelled backwards theeres an obstacle to avoid and doesn't make sense to get back in line with goal
            }
            else if (relax)
            {
    ...
    else
    {
        if (dir == LEFT || dir == RIGHT)
        {
            float angleL = this_start.q.GetAngle () + endCriteria.angle.get (),
                  angleR = this_start.q.GetAngle () - endCriteria.angle.get ();
            r.ended = (robotTransform.q.GetAngle () >= angleL
                       || robotTransform.q.GetAngle () <= angleR);
        }
...
EndedResult Task::checkEnded (const State &n, Direction dir, bool relax,
                              std::pair<bool, b2Transform> use_start)
{ //check error of node compared to the present Task
    EndedResult r;
    Angle a;
    Distance d;
...
bool Task::checkEnded (const b2PolygonShape &box,
                       const b2Transform &robot_pose, Disturbance *dist_obs)
{
    bool result = false;
    if (box.m_count < 4)
    {
    ...
    { //|| action.getOmega()!=0
        if (box.m_radius == 0 || action.getOmega () != 0)
        { //means that there is no goal
            b2Transform fromDi_start = from_Di (
                &b2Transform_zero, dist_obs); //transform at start of task
            b2Transform fromDi_now = from_Di (&b2Transform_zero);
            b2Transform inst_transform = b2MulT (
                fromDi_now,
                fromDi_start); //check how far Di has moved since start
            Angle a (fromDi_now.q.GetAngle ()
                     - (action.getTransform (LIDAR_SAMPLING_RATE / 4)
                            .q.GetAngle ())); //avoid turning too much!
            float _distance
                = std::max (inst_transform.p.Length (), start.p.Length ());
        ...
        else
        {
            ...
            for (int i = 0; i < box.m_count; i++)
            {
                fprintf (box_file, "%f\t%f\n", box.m_vertices[i].x,
                         box.m_vertices[i].y); //save predictions/
            }
            fclose (box_file);
            if (result)
            {
...
bool Task::isTurnFinished (const b2Transform &robotTransform, Direction dir)
{
    ...
    float angleL = start.q.GetAngle () + safeAngle,
          angleR = start.q.GetAngle () - safeAngle;
    ...
    bool finishedLeft
        = (round (robotAngle * 100) / 100)
          >= (round (angleL * 100) / 100); //-(action.getOmega()*HZ)/2;
    bool finishedRight
        = (round (robotAngle * 100) / 100)
          <= (round (angleR * 100) / 100); //+(action.getOmega()*HZ)/2;
