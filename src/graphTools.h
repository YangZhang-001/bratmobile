...
struct CompareValue
{
    ...
    bool operator() (const std::tuple<V, M, float> &p1,
                     const std::tuple<V, M, float> &p2) const
    {
...
struct Edge
{
    ...
    bool overrideZeroSteps
        = false; //set to true if an edge with no steps should not be considered a self-edge
...
struct State
{
    ...
    State (const b2Transform &_start, const Disturbance &di,
           const Direction &dir)
        : start (_start), Di (di), direction (dir)
    {
    }
    ...
    bool isTurning () const { return direction == LEFT || direction == RIGHT; }
...
struct StateDifference
{
    ...
    StateDifference (const State &s1, const State &s2) { init (s1, s2); }

    float sum () { return sum_r () + sum_D (Di) + sum_D (Dn); }
 ...
 */
    void fill_valid_bodyfeatures (BodyFeatures &bf, const State &s1,
                                  const State &s2, WHAT_D_FLAG flag);
};
...
typedef boost::graph_traits<TransitionSystem>::vertex_descriptor
    vertexDescriptor;
...
struct is_not_v
{
    ...
    bool operator() (edgeDescriptor e) { return e.m_target != cv; }
...
/**
	 ...
	 * @param it iteration
	 */
void update (edgeDescriptor e, std::pair<State, Edge> sk, TransitionSystem &g,
             bool current, int it);
/**
	 ...
	 * @param it iteration
	 */
void set (edgeDescriptor e, std::pair<State, Edge> sk, TransitionSystem &,
          bool current, int it);

std::pair<bool, edgeDescriptor>
getMostLikely (TransitionSystem &, std::vector<edgeDescriptor>, int);

std::vector<edgeDescriptor> outEdges (
    TransitionSystem &, vertexDescriptor,
    Direction); //returns a vector containing all the out-edges of a vertex which have the specified direction

Disturbance getExpectedDisturbance (TransitionSystem &, vertexDescriptor,
                                    Direction, int);
...
/**
	 ...
	 */
std::pair<bool, edgeDescriptor>
visitedEdge (const std::vector<edgeDescriptor> &es, TransitionSystem &g,
             vertexDescriptor cv = TransitionSystem::null_vertex ());

std::pair<edgeDescriptor, bool> add_edge (
    const vertexDescriptor &, const vertexDescriptor &, TransitionSystem &,
    const int &,
    Direction d
    = UNDEFINED); //wrapper around boost function, disallows edges to self
...
/**
	 ...
	 * @return false 
	 */
bool check_edge_direction (const std::pair<edgeDescriptor, bool> &,
                           TransitionSystem &, Direction);
/**
	 ...
	 */
std::vector<vertexDescriptor>::iterator to_task_end (
    edgeDescriptor &e, TransitionSystem &g,
    const std::vector<vertexDescriptor> &plan,
    std::vector<vertexDescriptor>::iterator
        it); //in a vector, finds vertices belonging to the same task and skips to the end fo the task
...
struct ViableEdge
{
    ...
    bool operator() (const edgeDescriptor &e) const
    {
        NotSelfEdge nse (g);
        bool not_self = nse (e)
                        || e.m_source == e.m_target && (*g)[e].step != 0
                               && (*g)[e].it_observed >= 0;
        return not_self;
    }
...
typedef boost::filtered_graph<TransitionSystem, ViableEdge, Connected>
    FilteredTS;
...
class StateMatcher
{
  public:
    ...
    struct StateMatch
    {
        ...
        bool exact () { return pose () && abstract (); }

        bool pose () { return position && angle; }

        bool abstract ()
        { //states match Di and Dn regardless of objective pose
            return Di_exact () && Dn_exact ();
        }

        bool Di_exact () { return Di_position && Di_angle && Di_shape; }

        bool Dn_exact () { return Dn_position && Dn_angle && Dn_shape; }

        bool Di_pose () { return Di_position && Di_angle; }

        bool Dn_pose () { return Dn_position && Dn_angle; }

        bool shape_Di () { return Di_shape; }

        bool shape_Dn () { return Dn_shape; }
        bool Dn () { return Dn_shape && Dn_pose (); }

        bool Di () { return Di_shape && Di_pose (); }

        bool D_shapes () { return Dn_shape && Di_shape; }

        StateMatch () = default;

        StateMatch (const StateDifference &sd, const Threshold &threshold,
                    float coefficient = 1)
        {
            position = sd.pose.p.Length ()
                       < (threshold.for_robot_position () * coefficient);
            angle
                = fabs (sd.pose.q.GetAngle ()) < threshold.for_robot_angle ();
            Bundle Dn = threshold.for_Dn (), Di = threshold.for_Di ();
            Dn_position
                = sd.Dn.pose.p.Length () < (Dn.radius () * coefficient);
            Di_position
                = sd.Di.pose.p.Length () < (Di.radius () * coefficient);
            Dn_angle = fabs (sd.Dn.pose.q.GetAngle ()) < Dn.get_angle ();
            Di_angle = fabs (sd.Di.pose.q.GetAngle ()) < Di.get_angle ();
            bool Dn_below_threshold_w
                = fabs (sd.Dn.width ()) < (Dn.get_width () * coefficient * 2);
            bool Dn_below_threshold_l = fabs (sd.Dn.length ())
                                        < (Dn.get_length () * coefficient * 2);
            bool Di_below_threshold_w
                = fabs (sd.Di.width ()) < (Dn.get_width () * coefficient * 2);
            bool Di_below_threshold_l = fabs (sd.Di.length ())
                                        < (Dn.get_length () * coefficient * 2);
        ...
        {
            if (exact ())
            { //match position and disturbance
                return _TRUE;
            }
            else if (abstract ())
            {
    ...
    MATCH_TYPE isMatch (const StateDifference &, const Threshold &,
                        float endDistance = 0); //endDistance=endpose

    MATCH_TYPE isMatch (
        const State &s, const State &candidate, const Threshold &,
        const State *src = NULL,
        StateDifference *_sd
        = NULL); //first state: state to find a match for, second state: candidate match
