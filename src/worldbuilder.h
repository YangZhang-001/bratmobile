...
class WorldBuilder
{
  ...
  public:
    ...
    bool checkDisturbance (Pointf, bool &, Task *curr = NULL,
                           float range = 0.025);
     ...
     * @param task 
     */
    virtual void buildWorld (b2World &, b2Transform, Direction,
    ...
    bounds (Direction, b2Transform t, float boxLength, float halfWindowWidth,
    ...
    b2Vec2 averagePoint (
        const CoordinateContainer &, Disturbance &,
        float rad
        = 0.025); //finds centroid of a poitn cluster, return position vec difference

    int getBodies () { return bodies; }

    void add_body_count () { bodies++; }

    void resetBodies () { bodies = 0; }

    void add_iteration (int i = 1) { iteration += i; }
     ...
     * @return b2AABB 
     */
    b2AABB makeRobotSensor (b2Body *const robotBody, const Disturbance &focus)
        const; //returns bounding box in world coord
...
{
  protected:
    virtual void buildWorld (b2World &w, b2Transform start, Direction d,
                             Disturbance disturbance, float halfWindowWidth,
                             CLUSTERING clustering, Task *task) override;
};
...
{
  protected:
    std::vector<BodyFeatures> getFeatures (const CoordinateContainer &current,
                                           b2Transform start,
                                           CLUSTERING clustering) override;
};
 ...
 */
class EveryOtherPointBuilder : public virtual EveryOtherFeatureBuilder,
                               public virtual EverythingBuilder
{
  protected:
    std::vector<BodyFeatures> getFeatures (const CoordinateContainer &current,
                                           b2Transform start,
                                           CLUSTERING clustering) override
    {
        return EveryOtherFeatureBuilder::getFeatures (current, start,
                                                      clustering);
    }

    void buildWorld (b2World &w, b2Transform start, Direction d,
                     Disturbance disturbance, float halfWindowWidth,
                     CLUSTERING clustering, Task *task) override
    {
        EverythingBuilder::buildWorld (w, start, d, disturbance,
                                       halfWindowWidth, clustering, task);
    }
};
 ...
 * 
 */
class LaserFocus : public virtual WorldBuilder
{ //legacy
  protected:
