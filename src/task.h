...
class Task
{
  ...
  protected:
    ...
    EndCriteria
        endCriteria; //end criteria other than task encounters a disturbance
 ...
 */
    struct Action
    {
      private:
        float linearSpeed
            = WHEEL_SPEED_DEFAULT
              * 2;       //used to calculate instantaneous velocity using omega
      ...
      public:
        ...
        void setVelocities (const float &l, const float &r)
        {
            omega
                = (MAX_SPEED * (r - l)
                   / BETWEEN_WHEELS); //instant velocity, determines angle increment in willcollide
     ...
     */
        ...
        float getRWheelSpeed () const { return R; }

        float getLWheelSpeed () const { return L; }

        void setRWheelSpeed (float f) { R = f; }

        void setLWheelSpeed (float f) { L = f; }

        bool isValid () { return valid; }

        float getLinearSpeed () { return linearSpeed; }

        float getOmega (const float &dt = 1) const { return omega * dt; }
     ...
     */
        ...
        {
            float result
                = (MAX_SPEED * (r - l) / BETWEEN_WHEELS) * TURN_FRICTION * dt;
            return result;
        }

        void setOmega (const float &o) { omega = o; }

        void setLinearSpeed (const float &s) { linearSpeed = s; }
    };
    ...
    class Listener : public b2ContactListener
    {
      ...
      public:
        ...
        void BeginContact (b2Contact *contact)
        {
            ...
            if (bodyData.pointer == ROBOT_FLAG)
            { //if fixtureA belongs to robot
            ...
            if (bodyData.pointer == ROBOT_FLAG)
            { //WAS IF BODYDATA.POINTER if fixtureB belongs to robot
        ...
        std::vector<b2Body *> get_collisions () { return collisions; }
    };

  public:
    Task::Action getAction () const { return action; }
 ...
 */
    Direction
    H (Disturbance ob, Direction d,
       bool topDown
       = 0); //topDown enables Configurator topdown control on reactive behaviour
 ...
 */
    void setEndCriteria (const Angle &angle = SAFE_ANGLE,
                         const Distance &distance = BOX2DRANGE);

    void setEndCriteria (const Distance &distance);

    void setEndCriteria (const EndCriteria &ec) { endCriteria = ec; }
 ...
 * @return EndedResult 
 */
    EndedResult checkEnded (b2Transform robotTransform = b2Transform_zero,
                            Direction dir = UNDEFINED, bool relax = 0,
                            b2Body *robot = NULL,
                            std::pair<bool, b2Transform> use_start
                            = std::pair<bool, b2Transform> (1,
                                                            b2Transform_zero));
 ...
 * @return EndedResult 
 */
    EndedResult checkEnded (
        const State &n, Direction dir = UNDEFINED, bool relax = false,
        std::pair<bool, b2Transform> use_start = std::pair<bool, b2Transform> (
            1, b2Transform_zero)); //usually used to check against control goal
 ...
 * @return false 
 */
    bool checkEnded (const b2PolygonShape &box,
                     const b2Transform &robot_pose = b2Transform_zero,
    ...
    Task (Disturbance ob, Direction d,
          b2Transform _start = b2Transform (b2Vec2 (0.0, 0.0), b2Rot (0.0)),
          bool topDown = 0)
    {
        start = _start;
        disturbance = ob;
 ...
 * @return simResult 
 */
    simResult bumping_that (b2World &_world, int iteration, b2Body *,
 ...
 */
    EndCriteria &getEndCriteria () { return endCriteria; }
 ...
 */
    b2Transform from_Di (
        ...
        Disturbance *d_obs
        = NULL); //d_obs disturbance observed rather than D with which task was init

    void set_change (bool b) { change = b; }
 ...
 * @return false 
 */
    bool is_over () { return change || motorStep == 0; }
 ...
 * @return Disturbance* 
 */
    Disturbance *get_disturbance_ptr () { return &disturbance; }

    const Disturbance &get_disturbance () const { return disturbance; }

    void setMotorStep (int i) { motorStep = i; }

    int getMotorStep () const { return motorStep; }

    b2Transform getStart () { return start; }

    b2Transform &getStartRef () { return start; }
