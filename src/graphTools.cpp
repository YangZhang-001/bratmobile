...
void StateDifference::init (const State &s1, const State &s2)
{                                               //observed, desired
    pose.p.x = s1.endPose.p.x - s2.endPose.p.x; //endpose x
    pose.p.y = s1.endPose.p.y - s2.endPose.p.y; //endpose y
    pose.q.Set (
        angle_subtract (s1.endPose.q.GetAngle (), s2.endPose.q.GetAngle ()));
                                               ...
                                               WHAT_D_FLAG flag)
{
    ...
    if (flag == DI)
    {
        ...
        p1 = s1.start_from_Di ();
        p2 = s2.start_from_Di ();
        bf.halfWidth = (s1.Di.bodyFeatures ().halfWidth
                        - s2.Di.bodyFeatures ().halfWidth);
        bf.halfLength = (s1.Di.bodyFeatures ().halfLength
                         - s2.Di.bodyFeatures ().halfLength);
    }
    else if (flag == DN)
    {
        ...
        p1 = s1.end_from_Dn ();
        p2 = s2.end_from_Dn ();
        bf.halfWidth = (s1.Dn.bodyFeatures ().halfWidth
                        - s2.Dn.bodyFeatures ().halfWidth);
        bf.halfLength = (s1.Dn.bodyFeatures ().halfLength
                         - s2.Dn.bodyFeatures ().halfLength);
    }
...
void gt::update (edgeDescriptor e, std::pair<State, Edge> sk,
                 TransitionSystem &g, bool current, int it)
{
    if (e == edgeDescriptor ())
    {
        return;
...
void gt::set (edgeDescriptor e, std::pair<State, Edge> sk, TransitionSystem &g,
              bool current, int it)
{
...
std::vector<edgeDescriptor> gt::outEdges (TransitionSystem &g,
                                          vertexDescriptor v, Direction d)
{
...
gt::getMostLikely (TransitionSystem &g, std::vector<edgeDescriptor> oe, int it)
{
...
Disturbance gt::getExpectedDisturbance (TransitionSystem &g,
                                        vertexDescriptor v, Direction d,
                                        int it)
{
...
std::pair<bool, edgeDescriptor>
gt::visitedEdge (const std::vector<edgeDescriptor> &es, TransitionSystem &g,
                 vertexDescriptor cv)
{
    ...
    if (possible_solutions.size () > 1)
    {
        ...
        auto it_recent = std::max_element (possible_solutions.begin (),
...
std::pair<edgeDescriptor, bool> gt::add_edge (const vertexDescriptor &u,
                                              const vertexDescriptor &v,
                                              TransitionSystem &g,
                                              const int &it, Direction d)
{
    ...
    if (d == DEFAULT && result.second)
    {
        ...
        g[result.first].step
            = distanceToSimStep (g[v].distance (), values.first);
...
bool gt::check_edge_direction (const std::pair<edgeDescriptor, bool> &ep,
                               TransitionSystem &g, Direction d)
{
    bool result = false;
    if (ep.second)
    {
...
gt::to_task_end (edgeDescriptor &e, TransitionSystem &g,
                 const std::vector<vertexDescriptor> &plan,
                 std::vector<vertexDescriptor>::iterator it)
{
...
bool StateMatcher::match_equal (const MATCH_TYPE &candidate,
                                const MATCH_TYPE &desired)
{
    ...
    case _TRUE:
        ...
        break;
    case DN_SHAPE:
        result = (int (candidate) == int (desired)) || int (candidate) == _TRUE
                 || int (candidate) == ABSTRACT || candidate == D_NEW;
        break;
    case DN_POSE:
        result = (int (candidate) == int (desired)) || candidate == _TRUE
                 || int (candidate) == ABSTRACT || candidate == D_NEW;
        break;
    case DI_SHAPE:
        result = (int (candidate) == int (desired)) || candidate == _TRUE
                 || candidate == ABSTRACT || candidate == D_INIT;
        break;
    case DI_POSE:
        result = (int (candidate) == int (desired)) || candidate == _TRUE
                 || candidate == ABSTRACT || candidate == D_INIT;
...
StateMatcher::MATCH_TYPE StateMatcher::isMatch (const StateDifference &sd,
...
StateMatcher::MATCH_TYPE StateMatcher::isMatch (const State &s,
