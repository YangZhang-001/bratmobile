 ...
 */
    ...
    void addIteration (int i = 1) { iteration += i; }
 ...
 */
    std::pair<edgeDescriptor, bool> addVertex (const vertexDescriptor &src,
                                               vertexDescriptor &v1,
                                               Edge edge = Edge (),
                                               bool topDown = 0);
 ...
 * 
 */
    float approximate_angle (float angle, Direction d,
 ...
 */
    ...
    Controller *get_controller () { return task_controller; }
    /**
...
*/
    ...
    Tracker *get_tracker () const { return tracker; }

    MotorInterface *get_motor_interface () { return control; }

    void register_planner (Planner *_p) { planner = _p; }

    void setSimulationStep (float f) { simulationStep = f; }

    void register_logger (Logger *l) { logger = l; }

    void register_goalChanger (GoalChanger *gc) { goal_changer = gc; }
...
class ReactiveConfigurator : public Configurator
{
  ...
  public:
    ...
    ReactiveConfigurator (Task _task) { init (_task); }
};
