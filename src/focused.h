 ...
 */
    ExecutionInfo package_info (vertexDescriptor gv
                                = TransitionSystem::null_vertex (),
                                bool been = false)
    {
        return ExecutionInfo (currentVertex, gv, currentTask, controlGoal,
                              been, m_plan);
    }
 ...
 * 		   goal Di otherwise 
 */
    virtual Disturbance getDisturbance (TransitionSystem &g,
                                        vertexDescriptor v, b2World &world,
                                        const Direction &dir,
 ...
 */
    virtual void backtrack (std::vector<vertexDescriptor> &evaluation_q,
 ...
 */
    ...
    splitTask (vertexDescriptor v, Direction d,
 ...
 */
    virtual VertexMatch
    hardMatch (State s, Direction dir = Direction::UNDEFINED,
               StateMatcher::MATCH_TYPE match_type = StateMatcher::_TRUE,
               StateDifference *_sd = NULL); //matches to most likely
 ...
 */
    virtual VertexMatch
    findMatch (State s, Direction dir = Direction::UNDEFINED,
               StateMatcher::MATCH_TYPE match_type = StateMatcher::_TRUE,
               StateDifference *_sd = NULL,
               vertexDescriptor src
               = TransitionSystem::null_vertex ()); //matches to most likely
 ...
 */
    virtual std::vector<vertexDescriptor> explorer (
        vertexDescriptor v, TransitionSystem &g,
        b2World
            &w); //evaluates only after DEFAULT, internal one step lookahead
 ...
 */
    ...
    add_vertex_now (const vertexDescriptor &src, vertexDescriptor &v1,
                    Disturbance obs, Edge edge = Edge (), bool topDown = 0);
 ...
 * @param topDown flag determining whether state v1 has been simulated already or not
 */
    std::pair<edgeDescriptor, bool> add_vertex_retro (vertexDescriptor &src,
                                                      vertexDescriptor &v1,
                                                      Edge edge = Edge (),
                                                      bool topDown = 0);
 ...
 */
    virtual bool preventTransition (vertexDescriptor v)
    {
        return v == currentVertex
               && (!m_plan.empty () || !currentTask.is_over ());
    }
...
*/
    virtual void transitionMatrix (vertexDescriptor v, Direction d,
                                   vertexDescriptor src);
 ...
 */
    void applyTransitionMatrix (vertexDescriptor v0, Direction d, bool ended,
                                vertexDescriptor src,
                                std::vector<vertexDescriptor> &plan_prov);
 ...
 */
    void addToPriorityQueue (vertexDescriptor v,
    ...
    void
    shift_states (TransitionSystem &, const std::vector<vertexDescriptor> &,
                  const b2Transform
                      &); //shifts a sequence of states by a certain transform
 ...
 */
    ...
    get_next_option (vertexDescriptor v, vertexDescriptor src,
 ...
 */
    std::pair<State, Edge>
    simulation_setup (b2World &w, Task &t, vertexDescriptor v0,
                      b2Transform shift, b2Transform &start,
 ...
 */
    std::pair<edgeDescriptor, bool>
    setup_match_edge (VertexMatch &match, vertexDescriptor &v0,
                      vertexDescriptor &v1, const Edge &k, Direction direction,
                      bool changedMatch);
 ...
 */
    virtual std::vector<vertexDescriptor>
    task_vertices (vertexDescriptor v,
 ...
 */
    vertexDescriptor getRecyclingStart (vertexDescriptor v,
                                        vertexDescriptor v1,
 ...
 */
    std::vector<edgeDescriptor> inEdges (
        vertexDescriptor v,
        Direction d
        = UNDEFINED); //returns a vector containing all the in-edges of a vertex which have the specified direction

    virtual bool closeVertex (std::set<vertexDescriptor> &closed,
                              vertexDescriptor v);
 ...
 */
    bool correctQueue (std::vector<vertexDescriptor> &queue,
                       vertexDescriptor v, vertexDescriptor startRecycle,
                       int planProvSize);
 ...
 */
    void abandonPlan (std::vector<vertexDescriptor> &planProv,
                      vertexDescriptor v0, vertexDescriptor v1);
 ...
 */
    ...
    partiallyExplorativeOptions (std::pair<bool, edgeDescriptor> ve);

    virtual Robot makeRobot (b2World &w, const Task &task) override;
    /**
 ...
 */
    ...
    {
      protected:
        vertexDescriptor lastAdded = TransitionSystem::
            null_vertex (); //last vertex added to the evaluation queue
      public:
        /**
	 ...
	 */
        void
        addToEvaluationQueue (std::vector<vertexDescriptor> &evaluationQueue,
                              vertexDescriptor v1, TransitionSystem &g,
                              vertexDescriptor v);
 ...
 */
    void visitedDirectionsPushBack (vertexDescriptor v,
 ...
 */
    virtual bool shouldPartiallyExplore (const std::vector<edgeDescriptor> &oe,
 ...
 */
    ...
    virtual float customSimulationStep (vertexDescriptor v
                                        = TransitionSystem::null_vertex ());
  ...
  public:
    ...
    FocusedConfigurator (Task _task) { init (_task); }
