...
EndedResult estimateCost (const State &state, b2Transform start, Direction d,
                          Task &_goal)
{
...
void HorizonStarPlanner::path2add2 (
    ...
    std::vector<std::vector<vertexDescriptor> > &paths, TransitionSystem &g)
{
    std::pair<edgeDescriptor, bool> edge (edgeDescriptor (), false);
    std::vector<vertexDescriptor>::reverse_iterator path_end_rit
        = (path->rbegin ()); //reverse iterator to end of path
    while (!edge.second)
    {
        vertexDescriptor end
            = *(path_end_rit.base () - 1); //equivalent to path.end()
        edge = boost::edge (end, add[0], g);
        if (!add.empty () && !edge.second && path != paths.rend ())
        {   //if this path does not have an edge and there are
            //other possible paths, go to previous paths
            if (path_end_rit.base () - 1 != (path->begin ()))
            { //if the current vertex is not the root of the path
            ...
            else
            {
                ...
                path_end_rit = (*path).rbegin (); //reset the path end
                 ...
                 && path_end_rit.base () != path->rbegin ().base ())
        { //if there is an edge with the end of current path
            bool found = 0;
            ...
            struct HasStartStop
            {
                vertexDescriptor start = TransitionSystem::null_vertex (),
                                 stop = TransitionSystem::null_vertex ();
                HasStartStop (vertexDescriptor _start, vertexDescriptor _stop)
            ...
            auto _p = std::find_if (
                paths.rend (), paths.rbegin (),
                HasStartStop (*path->begin (), *path_end_rit.base ()));
            if (_p != paths.rbegin ())
            {
                path = _p;
            }
            else
            {
                ...
                paths.emplace_back (std::vector<vertexDescriptor> (
                    path->begin (), path_end_rit.base ()));
                path = paths.rbegin ();
            }
            break;
        }
        if (path == paths.rend ())
        { //if there are no other paths
            paths.push_back (
                std::vector<vertexDescriptor> ()); //make a new one
            path = paths.rbegin ();
            break;
        }
    }
    ...
    const TransitionSystem &g)
{
    ...
    //LAMBDA
    auto skip_first = [] (const std::vector<vertexDescriptor> &_plan,
                          const vertexDescriptor &_cv,
                          const TransitionSystem &_g, const bool &_change) {
        bool empty_xor_currentv
            = ( ( _plan.size () == 1 )
               ^ (_plan[0] != _cv && _plan[0] != MOVING_VERTEX));
        if (empty_xor_currentv && _change)
        { //&& _plan[0]==_cv
...
std::vector<Frontier>
frontierVertices (vertexDescriptor v, TransitionSystem &g, ExecutionInfo &info)
{
    std::vector<Frontier> result;
    std::pair<edgeDescriptor, bool> ep = boost::edge (MOVING_VERTEX, v, g);
    vertexDescriptor v0 = v, v1 = v, v0_exp;
    //do{
    if ((info.overarchingGoal ().get_disturbance ().getPosition ()
         - g[v].endPose.p)
            .Length ()
        >= DISTANCE_ERROR_TOLERANCE)
    {
        ...
        for (auto ei = es.first; ei != es.second; ei++)
        {
            ...
            do
            {
                ...
                if ((g[(*ei3).m_target].visited () || info.been ())
                    && g[(*ei3)].it_observed >= 0
                    && (not_self_edge (*ei3) || g[*ei3].overrideZeroSteps))
                { //(*ei3).m_source!=(*ei3).m_target
                    ...
                    {
                        EndedResult er = estimateCost (
                            g[(*ei3).m_target], g[(*ei3).m_source].endPose,
                            g[(*ei3).m_target].direction,
                            info.overarchingGoal ());
                        std::vector<vertexDescriptor> _plan = info.plan ();
                        g[(*ei3).m_target].phi
                            = evaluationFunction (er, (*ei3).m_target, _plan);
                    }
                    if (g[(*ei3).m_target].direction == DEFAULT)
                ...
                { //not sure if this is right just added
                    break;
                }
                if (ei3 == es3.second)
                {
...
void HorizonStarPlanner::addToPriorityQueue (
    ...
    const std::set<vertexDescriptor> &closed, vertexDescriptor goal)
{
                                                        ...
                                                        bool *finished)
{
    ...
    std::vector<Frontier> priorityQueue
        = { Frontier (src, std::vector<vertexDescriptor> ()) };
    // costMap.init(g);
    int no_out = 0;
    ...
    do
    {
        frontier_v = frontierVertices (
            src, g,
            info); // get next default tasks (plus non-default connecting tasks)
        closed.emplace (src);
        priorityQueue.erase (priorityQueue.begin ());
        for (Frontier f : frontier_v)
        ...
        if (!priorityQueue.empty ())
        {
            ...
            add = std::vector<vertexDescriptor> (
                priorityQueue.begin ()->connecting.begin (),
                priorityQueue.begin ()
                    ->connecting.end ()); //lowest phi frontier
            add.push_back (src);
            path2add2 (path, add, paths,
                       g); //find path to add frontier (add) to
        ...
        _finished = info.overarchingGoal ()
                        .checkEnded (g[path_end].endPose, UNDEFINED, true)
                        .ended;
        if (NULL != finished)
        {
            *finished = _finished;
    ...
    {
        return best_path (paths, goal, info.currentVertex (),
                          info.currentTask ().get_change (), g);
    }
    if (!_finished)
    {
