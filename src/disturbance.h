...
typedef unsigned int
    AffordanceIndex; //was thinking of this being a character but doesn't have to be maybe enum is fine
...
struct BodyFeatures
{
    ...
    bool attention
        = false; //attention is used to indicate that the disturbance is the initial disturbance of a task and is being tracked by the robot using the attention window
    ...
    */
    bool match (const BodyFeatures &, Bundle *bundle = NULL,
                b2Transform t = b2Transform_zero) const;
    ...
    Bundle get_bundle ()
    {
        return Bundle (pose.p.x, pose.p.y, pose.q.GetAngle (), halfWidth,
                       halfLength);
    }
};
...
struct Disturbance
{
  ...
  protected:
    ...
    AffordanceIndex affordanceIndex
        = NONE; //not using the enum because in the future we might want to add more affordances
    bool valid = 0;
    bool rotation_valid = 0;

    void setOrientation (float f)
    { //returns orientation (angle) of a point, in order
        rotation_valid = 1;
        bf.pose.q.Set (f);
    }
  ...
  public:
    BodyFeatures bf
        = BodyFeatures (b2Transform (b2Vec2 (10000, 10000), b2Rot (M_PI)));

    Disturbance (){};
    Disturbance (AffordanceIndex i) { affordanceIndex = i; }
    ...
    Disturbance (BodyFeatures _bf) : bf (_bf) { affordanceIndex = AVOID; }
    ...
    Disturbance (b2Body *b)
    {
        ...
        if (bf.shape == b2Shape::e_polygon)
        {
            ...
            std::vector<b2Vec2> local_vertices
                = arrayToVec (poly->m_vertices, poly->m_count);
            CompareX compareX;
            CompareY compareY;
            float minx = (std::min_element (local_vertices.begin (),
                                            local_vertices.end (), compareX))
                             .base ()
                             ->x;
            float miny = (std::min_element (local_vertices.begin (),
                                            local_vertices.end (), compareY))
                             .base ()
                             ->y;
            float maxx = (std::max_element (local_vertices.begin (),
                                            local_vertices.end (), compareX))
                             .base ()
                             ->x;
            float maxy = (std::max_element (local_vertices.begin (),
                                            local_vertices.end (), compareY))
                             .base ()
                             ->y;
    ...
    float getAngle (b2Body *b) { return getAngle (b->GetTransform ()); }

    void setPosition (b2Vec2 pos) { bf.pose.p.Set (pos.x, pos.y); }

    void setPosition (float x, float y) { bf.pose.p.Set (x, y); }

    b2Vec2 getPosition () const { return bf.pose.p; }

    bool isValid () const { return valid; }

    AffordanceIndex getAffIndex () const { return affordanceIndex; }

    void set_affordance (AffordanceIndex a) { affordanceIndex = a; }
    ...
    b2Transform pose () const { return bf.pose; }

    void setPose (b2Transform t) { bf.pose = t; }
    ...
    BodyFeatures bodyFeatures () const { return bf; }
    ...
    float halfLength () { return bf.halfLength; }

    float halfWidth () { return bf.halfWidth; }
...
struct simResult
{
    enum resultType
    {
        ...
        crashed = 1,
        safeForNow = 2
    }; //successful =0, crashed =1, safeForNow=2
    ...
    simResult (resultType code, Disturbance obst)
        : resultCode (code), collision (obst)
    {
        // valid =1;
    }
};
