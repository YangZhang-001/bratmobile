...
{
    FrontierCrashed () = delete;

    FrontierCrashed (TransitionSystem &ts, Direction d)
        : transitionSystem (ts), direction (d)
    {
    }

    bool operator() (const Frontier &f) const
    {
        if (f.connecting.empty ())
            return false;
        return transitionSystem[f.connecting[0]].direction == direction
               && transitionSystem[f.frontier].outcome == simResult::crashed;
    }
...
class B2BConfigurator : public virtual AttentiveConfigurator
{
  public:
    ...
    B2BConfigurator (Task &task) { init (task); }
  ...
  protected:
     ...
     */
    bool attentionWindowOverlaps (const Disturbance &Di, const State &q,
                                  b2World &world, const Disturbance &focus);
     ...
     */
    template <typename P>
    std::vector<Direction> transitionInHindsight (vertexDescriptor v,
                                                  P predicate)
    {
 ...
 */
    ...
    explorer (vertexDescriptor v, TransitionSystem &g, b2World &w)
        override; //evaluates only after DEFAULT, internal one step lookahead
 ...
 */
    class ClearVoyance
    {
      public:
        struct DisturbanceLookahead
        {
            ...
            vertexDescriptor
                source; //vertex from which additional DEFAULT Tasks will be explored

            DisturbanceLookahead () = default;

            DisturbanceLookahead (vertexDescriptor v, const Disturbance &d)
                : source (v)
            {
     ...
     */
        ...
        void setRoot (vertexDescriptor v) { root = v; }

        vertexDescriptor getRoot () { return root; }

      protected:
        vertexDescriptor root
            = TransitionSystem::null_vertex (); //root of modular expansion
 ...
 * @param clearvoyance 
 */
    void addOptionsInHindsight (vertexDescriptor v, vertexDescriptor v0,
                                vertexDescriptor v1);
};
