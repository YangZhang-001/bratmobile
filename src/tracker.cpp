...
bool DeadReckoner::hasTaskEnded(Task & t, const State & s){
    if (s.isTurning()){
        return s.endPose.q.c>t.getAction().getTransform(LIDAR_SAMPLING_RATE/2).q.c; //closer to zero
    }
    else{
        return s.endPose.p.x>=0.01; //within 1cm from expected endPose?
    }
    return t.getMotorStep()<1;
}

bool ClosedLoop_Tracker::hasTaskEnded(Task & t, const State & s){
