...
TrackingResult
Tracker::get_transform (const Task &t, const CoordinateContainer &pts,
                        const std::vector<BodyFeatures> &objects)
{
    ...
    {
        result.displacement
            = t.getAction ().getTransform (LIDAR_SAMPLING_RATE);
    }
    result.observed_disturbance = t.get_disturbance ();
    result.observed_disturbance.setPose (
        b2help::InvMul (result.displacement, t.get_disturbance ().pose ()));
    return result;
}

bool Tracker::hasTaskEnded (Task &t) { return t.getMotorStep () < 1; }

bool ClosedLoop_Tracker::hasTaskEnded (Task &t)
{
    //check if task has ended according to disturbance
    bool ended = t.checkEnded (
        attention_window, b2Transform_zero,
        &tracked_disturbance); //the attention_window moves with the robot
...
TrackingResult
ClosedLoop_Tracker::track (const Task &t, const CoordinateContainer &pts,
                           const std::vector<BodyFeatures> &objects)
{
    TrackingResult result = get_transform (t, pts, objects);
    if (!hasReading)
...
cv::Rect2f ClosedLoop_Tracker::real_world_focus (const Task *t)
{
    ...
    float max_dimension = std::max (t->get_disturbance ().bf.width (),
                                    t->get_disturbance ().bf.length ());
    max_dimension += 0.02;
...
TrackingResult
ClosedLoop_Tracker::get_transform (const Task &t,
                                   const CoordinateContainer &pts,
                                   const std::vector<BodyFeatures> &objects)
{
    TrackingResult result = Tracker::get_transform (t, pts, objects);
    if (t.get_disturbance ().getAffIndex () == NONE
        || t.get_disturbance ().bf.is_point ()
        || (t.getAction ().getLWheelSpeed () == 0
            && t.getAction ().getRWheelSpeed () == 0))
    {
        if (t.get_disturbance ().getAffIndex () == NONE)
        {
        ...
        if ((t.getAction ().getLWheelSpeed () == 0
             && t.getAction ().getRWheelSpeed () == 0))
        {
    ...
    auto new_d_it
        = find_disturbance (objects.cbegin (), objects.cend (),
                            result.observed_disturbance.bodyFeatures (),
                            t.getAction ().getTransform (LIDAR_SAMPLING_RATE));
    if (new_d_it == objects.end ())
    {
        printf ("not found!");
        result.observed_disturbance.set_affordance (
            NONE); //this will tell the task that D is null, so it can end!
        return result;
    }
    ...
    correctAngle (result.observed_disturbance.bf,
                  t.get_disturbance ().bodyFeatures ());
    calc_transform (result.displacement, result.observed_disturbance.pose (),
                    t.get_disturbance ().pose ());
...
std::vector<BodyFeatures>::const_iterator
ClosedLoop_Tracker::find_disturbance (
    std::vector<BodyFeatures>::const_iterator objects_begin,
    std::vector<BodyFeatures>::const_iterator objects_end,
    const BodyFeatures &dist, b2Transform t, float *_least_square)
{
    ...
    while (objects_begin != objects_end)
    {
        ...
        if (float ss = distance.sum_squares () < least_square)
        {
            ...
            else if (Disturbance d (*objects_begin);
                     overlaps (attention_window, &d))
            {
                result = objects_begin;
                //adjust threshold
                ...
                printf ("overlaps but theres error! DISTANCE! x=%f "
                        "\ty%f\ttheta=%f\tw=%f\tl%f\t",
                        distance.get_x (), distance.get_y (),
                        distance.get_angle (), distance.get_width (),
                        distance.get_length ());
            }
        }
        objects_begin++;
...
{
    if (fabs (found.pose.q.GetAngle () - dist.pose.q.GetAngle ())
        > (3 * M_PI_4))
    {
                                              ...
                                              const Task &currentTask)
{
    ...
    if (currentTask.get_disturbance ().getAffIndex () == PURSUE
        && goal.get_disturbance ().getAffIndex ())
    {
        goalD = Disturbance ();
    }
    try
    {
        attention_window
            = sensor_box (Robot::get_vertices (), b2Transform_zero, goalD);
        if (area = window_area ();
            area < (ROBOT_HALFLENGTH * 2) * (ROBOT_HALFWIDTH * 2))
        {
            throw area;
        }
    }
