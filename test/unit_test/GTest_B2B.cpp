...
TEST_F (DebugB2BOptions, ApplyTransitionInHindsight)
{
    ...
    transitionSystem[4].outcome = simResult::crashed;
    transitionSystem[6].outcome = simResult::crashed;
    applyTransitionMatrix (currentVertex, DEFAULT, false, currentVertex,
                           m_plan);
    EXPECT_EQ (transitionSystem[currentVertex].options.size (), 2);
}
...
TEST_F (DebugB2BTest, Add180TurnToClearvoyance)
{
    ...
    auto v1 = make_v1_crashed (v0, b2Transform_zero, b2Transform_zero,
                               generateGoal ().pose ())
                  .m_target;
    setAllVisited ();
...
TEST_P (DebugB2BTestSplit, splitTask)
{
    ...
    {
        v1 = make_successful (currentVertex, std::get<1> (GetParam ()))
                 .m_target;
...
INSTANTIATE_TEST_CASE_P (SplitTask, DebugB2BTestSplit,
                         ::testing::Combine (::testing::Bool (),
                                             ::testing::Values (DEFAULT, LEFT,
                                                                RIGHT)));
...
INSTANTIATE_TEST_CASE_P (
    DisturbanceIsGoal, B2BTestGetGoal,
    ::testing::Values (
...
INSTANTIATE_TEST_CASE_P (
    DisturbanceIsObstacle, B2BTestGetObstacle,
    ::testing::Values (
        std::tuple<b2Transform, Direction, Direction> (
...
TEST_F (B2BTestGetObstacle, AvoidNoGoal)
{
    init (Task ());
    EXPECT_FALSE (
        controlGoal.get_disturbance ().isValid ()); //test case health check
...
TEST_F (DebugB2BTest, ClearVoyance)
{
    ...
    b2World world (b2Vec2 (0, 0));
    BodyFeatures bf = bodyFeatures (.55, 0, 0, 0.02, 0.05),
                 bf2 = bodyFeatures (0, .55, 0, 0.02, 0.05),
                 bf3 = bodyFeatures (0, -0.55, 0, 0.02, 0.05);
    //bf.attention=1;
    make_module (MOVING_VERTEX);
    transitionSystem[MOVING_VERTEX].Di
        = Disturbance (bf); //current task was avoiding
    ...
    vertex_options_push_back (MOVING_VERTEX, DEFAULT);
    Disturbance Di
        = getDisturbance (transitionSystem, MOVING_VERTEX, world, DEFAULT,
                          transitionSystem[MOVING_VERTEX].endPose);
    EXPECT_EQ (Di.bf.pose.p.x, transitionSystem[3].Dn.bf.pose.p.x);
    EXPECT_EQ (Di.bf.pose.p.y, transitionSystem[3].Dn.bf.pose.p.y);
    EXPECT_EQ (Di.bf.pose.q.GetAngle (),
               transitionSystem[3].Dn.bf.pose.q.GetAngle ());
...
TEST_F (DebugB2BTest, BacktrackCollision)
{
    ...
    backtrack (evaluation_q, priority_q, closed, plan_prov, currentVertex,
               currentVertex);
    EXPECT_FALSE (std::find_if (priority_q.begin (), priority_q.end (),
                                [this] (vertexDescriptor v) { return v == 1; })
                  == priority_q.end ());
}
...
{
    const char *info = ::testing::UnitTest::GetInstance ()
                           ->current_test_info ()
                           ->value_param ();
        ...
        = configurator->getGoal ()
              .checkEnded (
                  configurator
                      ...
                      .endPose)
              .ended;
    bool success = planned_to_goal
                   || configurator->getGoal ()
                          .checkEnded (configurator->vertex_get_endPose (
                              configurator->get_current_vertex ()))
                          .ended;
    EXPECT_TRUE (success);
}
...
TEST_P (HighLevelTestB2B, RecycleB2B)
{
    ...
    configurator->getTask ().set_change (true);
    EXPECT_EQ (configurator->getTask ().get_direction (),
               configurator->vertex_get_direction (last_v));
    EXPECT_TRUE (configurator->getTask ().get_disturbance ()
                 == configurator->vertex_get_Di (last_v));
...
INSTANTIATE_TEST_CASE_P (
    CulDeSac, HighLevelTestB2B,
    ::testing::Combine (::testing::Values (false),
                        ::testing::Values (std::string ("../cul_de_sac/")),
                        ::testing::Values (2, 3, 4, 17, 36)));

INSTANTIATE_TEST_CASE_P (
    Target40, HighLevelTestB2B,
    ::testing::Combine (::testing::Values (true),
                        ::testing::Values (std::string ("../target_40cm/")),
                        ::testing::Values (2, 3, 4, 6, 17, 89)));

INSTANTIATE_TEST_CASE_P (
    Target68, HighLevelTestB2B,
    ::testing::Combine (::testing::Values (true),
                        ::testing::Values (std::string ("../target_68cm/")),
                        ::testing::Values (2, 3, 4, 6, 17, 36)));

TEST_P (DebugB2BTestVertex, ClearVoyanceTurn)
{ //test clearvoyance when turning on the spot
    //GTEST_SKIP();
    init (Task ());
    EXPECT_FALSE (
        controlGoal.get_disturbance ().isValid ()); //test case health check
    ...
    // Disturbance solution=transitionSystem[MOVING_VERTEX].Di;
    transitionSystem[MOVING_VERTEX].direction = STOP;
    vertex_options_push_back (
        v0, vertex_get_direction (GetParam ())); //add option same turn
    Disturbance Di = getDisturbance (transitionSystem, v0, world,
                                     vertex_get_direction (GetParam ()),
                                     transitionSystem[v0].endPose);
    EXPECT_EQ (Di.bf.pose.p.x, transitionSystem[v1].Dn.bf.pose.p.x);
    EXPECT_EQ (Di.bf.pose.p.y, transitionSystem[v1].Dn.bf.pose.p.y);
    EXPECT_EQ (Di.bf.pose.q.GetAngle (),
               transitionSystem[v1].Dn.bf.pose.q.GetAngle ());
...
INSTANTIATE_TEST_CASE_P (TurningVertices, DebugB2BTestVertex,
                         ::testing::Values (2, 4));
...
TEST_F (HighLevelTestB2B, TrickyScenarioB2B)
{
    ...
    configurator->get_worldbuilder ()->set_world_objects (
        CreativeWorldBuilder::makeTricky ());
    b2World world (GRAVITY);
...
TEST_F (HighLevelTestB2B, TrappedB2B)
{
    ...
    configurator->get_worldbuilder ()->add_iteration ();
    configurator->get_worldbuilder ()->set_world_objects (
        CreativeWorldBuilder::makeTrickyTrap (.35));
    b2World world (GRAVITY);
    configurator->explorePlan (world);
    EXPECT_GT (configurator->get_plan ().size (), 0);
    EXPECT_TRUE (has180Turn (configurator->get_plan ()));
