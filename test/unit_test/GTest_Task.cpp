...
class TaskTest : public Task
{
  ...
  public:
    void TearDown () { taskInit (Disturbance (), DEFAULT); }

    EndCriteria taskInit (const Disturbance &_disturbance,
...
class TaskTestEndCriteria : public Task,
...
TEST_P (TaskTestEndCriteria, EndCriteriaAngleSign)
{
    ...
    {
    case LEFT:
        EXPECT_GT (endCriteria.angle.get_signed (), 0);
        break;
    case RIGHT:
        EXPECT_LT (endCriteria.angle.get_signed (), 0);
        break;
    default:
        EXPECT_FALSE (endCriteria.angle.isValid ());
        break;
    }
}
...
TEST_P (TaskTestEndCriteria, EndCriteriaAngleSignTarget)
{
    ...
    {
    case LEFT:
        EXPECT_EQ (endCriteria.angle.get_signed (), 0);
        break;
    case RIGHT:
        EXPECT_EQ (endCriteria.angle.get_signed (), 0);
        break;
...
INSTANTIATE_TEST_CASE_P (Directions, TaskTestEndCriteria,
                         testing::Values (LEFT, RIGHT, DEFAULT));

TEST_P (TaskTestEndCriteria, TaskHasFixedEndCriteria)
{
    Direction _direction = GetParam ();
    EndCriteria ec = taskInit (Disturbance (AVOID, b2Vec2 (0.5, 0), 0),
                               _direction); //tracked d
...
class ConfiguratorTestTask : public DebugConfigurator,
                             public TaskTestEndCriteria
{
};
...
TEST_P (ConfiguratorTestTask, AdjustSimTask)
{
    ...
    if (GetParam () != DEFAULT)
    {
        EXPECT_EQ (endCriteria.angle.get (),
                   ec.angle.get ()
                       - fabs (cltracker.getDeltaTransform ().q.GetAngle ()));
    }
...
INSTANTIATE_TEST_CASE_P (Directions, ConfiguratorTestTask,
                         testing::Values (LEFT, RIGHT, DEFAULT));

class TaskTestTermination
    : public TaskTest,
...
TEST_P (TaskTestTermination, TerminateEarly)
{
    ...
    EXPECT_NEAR (robot.body ()->GetTransform ().q.GetAngle (), angle,
                 M_PI / (2 * HZ));
}
...
INSTANTIATE_TEST_CASE_P (TerminateEarly, TaskTestTermination,
                         ::testing::Combine (testing::Values (LEFT, RIGHT,
                                                              DEFAULT),
                                             ::testing::Values (M_PI_4)));
INSTANTIATE_TEST_CASE_P (
    TerminateLate, TaskTestTermination,
    ::testing::Combine (testing::Values (LEFT, RIGHT, DEFAULT),
                        ::testing::Values (M_PI_4 + M_PI_2)));
