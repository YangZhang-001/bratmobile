...
TEST_F (FigureTest, PrintTransform)
{
    ...
    sprintf (info, "%0.3f-%0.3f-%0.3f.txt", b2d_transform.p.x,
             b2d_transform.p.y, b2d_transform.q.GetAngle ());
    Task goal;
...
TEST_P (HighLevelInterruptTest, CheckNoisyPlan)
{
    ...
    int iteration = std::get<2> (GetParam ()),
        taskToInterrupt = std::get<3> (GetParam ());
    // trackFor(iteration);
    Pointf interruptingPoint;
    std::vector<vertexDescriptor> updated_plan = get_InterruptedPlan (
        folder, iteration - 1, taskToInterrupt, &interruptingPoint); //map 2
    int vertices_now = configurator->n_vertices ();
    ...
    bool success = planned_to_goal
                   || configurator->getGoal ()
                          ...
                          .ended;
    Disturbance interruptingDisturbance (
        AVOID, b2Vec2 (interruptingPoint.x, interruptingPoint.y), 0);
    b2World world (GRAVITY);
    configurator->get_worldbuilder ()->buildWorld (world, b2Transform_zero,
                                                   DEFAULT);
    Robot robot (&world);
...
INSTANTIATE_TEST_CASE_P (
    CulDeSacTurning, HighLevelInterruptTest,
    testing::Combine (::testing::Values (false),
                      ::testing::Values (std::string ("../cul_de_sac/")),
                      ::testing::Values (2), ::testing::Values (-1, 0)));
//synth fails, debug later!

INSTANTIATE_TEST_CASE_P (
    CulDeSacAvoided, HighLevelInterruptTest,
    testing::Combine (::testing::Values (false),
                      ::testing::Values (std::string ("../cul_de_sac/")),
                      ::testing::Values (30), ::testing::Values (-1)));
...
TEST_F (HighLevelTest, Trapped)
{
    ...
    configurator->get_worldbuilder ()->add_iteration ();
    configurator->get_worldbuilder ()->set_world_objects (
        CreativeWorldBuilder::makeTrickyTrap (.3));
    b2World world (GRAVITY);
    configurator->explorePlan (world);
