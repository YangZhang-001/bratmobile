...
TEST (Robot, Vertices)
{
    ...
    EXPECT_NEAR (aabb.lowerBound.y, -ROBOT_HALFWIDTH - ROBOT_BOX_OFFSET_X,
                 0.01);
...
{
    b2World world (GRAVITY);
    BodyFeatures bf (b2Transform (b2Vec2 (1, 0), b2Rot (0))),
        bf2 (b2Transform (b2Vec2 (0.5, 0), b2Rot (0)));
 ...
 * 
 */
class ThirdPartyWB : public ::testing::Test,
                     public testing::WithParamInterface<int>
{
  public:
     ...
     */
    void make_culdesac (std::vector<cv::Point2f> &pts, int pts_per_side,
                        std::vector<cv::Point2f> *sideL = NULL,
                        std::vector<cv::Point2f> *sideR = NULL,
                        std::vector<cv::Point2f> *front = NULL)
    {
        float y = 0.05, x = 0;
...
INSTANTIATE_TEST_CASE_P (cds_sizes, ThirdPartyWB,
