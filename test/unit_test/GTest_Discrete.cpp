...
class DiscreteConfPlanTest
    : public DiscreteConfigurator,
...
class DiscreteCTest : public DiscreteConfiguratorTest,
...
TEST_F (DiscreteCTest, Replan)
{
    ...
    update_graph (transitionSystem,
                  TrackingResult (currentTask.get_disturbance (), dp));
...
class DiscreteCrashedTest : public DiscreteConfiguratorTest,
...
TEST_P (DiscreteCrashedTest, EstimateInCollision)
{
    ...
    if (GetParam () > .15)
    {
        ...
        if (m_plan.size () > 0)
        {
            ...
            EXPECT_TRUE (
                controlGoal.checkEnded (transitionSystem[plan_end]).ended);
        }
    }
}

class DiscretePropagateTest : public DiscreteConfiguratorTest,
...
TEST_P (DiscretePropagateTest, Propagate)
{
    ...
    if (GetParam () == DEFAULT)
    {
        ...
        EXPECT_TRUE (transitionSystem[currentVertex].Dn.getAffIndex ()
                     == AVOID);
    }
    else
    {
        ...
        EXPECT_FALSE (transitionSystem[currentVertex].Dn.getAffIndex ()
                      == AVOID);
    }
}

INSTANTIATE_TEST_CASE_P (Directions, DiscretePropagateTest,
...
TEST_F (DiscreteCTest, RemainingTime)
{
    ...
    for (int i = 0; i < 27; i++)
    {
        ...
        transitionSystem[v1].endPose = b2help::InvMul (
            currentTask.getAction ().getTransform (MOTOR_CALLBACK),
...
TEST_F (DiscreteCTest, Correct)
{
    ...
    data2fp.emplace (Pointf (point.x, point.y));
    update_graph (transitionSystem,
                  TrackingResult (currentTask.get_disturbance (),
                                  b2Transform (b2Vec2 (), b2Rot (angle))));
...
INSTANTIATE_TEST_CASE_P (XPosition, DiscreteCrashedTest,
                         ::testing::Values (0.16, 0.12, 0.10));
