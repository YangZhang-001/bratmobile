...
TEST (Initialisation, DebugConstructor) { DebugConfigurator configurator; }

TEST_F (ConfiguratorTest, Initialisation) {}

TEST (Initialisation, Task) { Task task; }
...
TEST_F (ConfiguratorTest, DummyVertex)
{
    ...
    EXPECT_FALSE (
        boost::edge (MOVING_VERTEX, MOVING_VERTEX, transitionSystem).second);
}
...
TEST_P (ConfiguratorTestGetGoal, GetDisturbanceGoal)
{
    ...
    Disturbance Di = getDisturbance (transitionSystem, currentVertex, world,
                                     std::get<2> (GetParam ()),
                                     transitionSystem[currentVertex].endPose);
...
INSTANTIATE_TEST_CASE_P (
    DisturbanceIsGoal, ConfiguratorTestGetGoal,
    ::testing::Values (
        std::tuple<b2Transform, Direction, Direction> (
            b2Transform (b2Vec2 (0.82, 0.31), b2Rot (0)), DEFAULT, DEFAULT),
        std::tuple<b2Transform, Direction, Direction> (
            b2Transform (b2Vec2 (0.82, 0.31), b2Rot (0)), DEFAULT, RIGHT),
        std::tuple<b2Transform, Direction, Direction> (
            b2Transform (b2Vec2 (0.82, 0.31), b2Rot (0)), LEFT, DEFAULT),
        std::tuple<b2Transform, Direction, Direction> (
            b2Transform (b2Vec2 (0.82, 0.31), b2Rot (0)), RIGHT, DEFAULT),
        std::tuple<b2Transform, Direction, Direction> (
            b2Transform (b2Vec2 (0.80, 0.0), b2Rot (-M_PI_2)), DEFAULT,
            DEFAULT),
        std::tuple<b2Transform, Direction, Direction> (
            b2Transform (b2Vec2 (0.80, 0.0), b2Rot (-M_PI_2)), DEFAULT, RIGHT),
        std::tuple<b2Transform, Direction, Direction> (
            b2Transform (b2Vec2 (0.80, 0.0), b2Rot (-M_PI_2)), DEFAULT,
            LEFT)));
...
INSTANTIATE_TEST_CASE_P (
    DisturbanceIsObstacle, ConfiguratorTestGetObstacle,
    ::testing::Values (
        std::tuple<b2Transform, Direction, Direction> (
            b2Transform (b2Vec2 (0.4, 0.0), b2Rot (M_PI_2)), LEFT, DEFAULT),
        std::tuple<b2Transform, Direction, Direction> (
            b2Transform (b2Vec2 (0.4, 0.0), b2Rot (M_PI_2)), RIGHT, DEFAULT),
        std::tuple<b2Transform, Direction, Direction> (
            b2Transform (b2Vec2 (0.40, 0.31), b2Rot (-M_PI_2)), RIGHT,
            DEFAULT)));
...
TEST_F (ConfiguratorTestGetObstacle, CrashToGoal)
{
    b2World world (b2Vec2 (0, 0));
    BodyFeatures bf = bodyFeatures (.55, 0, 0, 0.02, 0.05);
    transitionSystem[currentVertex].Di
        = Disturbance (PURSUE, b2Vec2 (1.0, 0)); //current task was avoiding
    transitionSystem[currentVertex].Dn
        = Disturbance (bf); //current task was avoiding
    ...
    Disturbance Di
        = getDisturbance (transitionSystem, currentVertex, world, LEFT,
                          transitionSystem[currentVertex].endPose);
    EXPECT_EQ (Di.bf.pose.p.x, solution.bf.pose.p.x);
    EXPECT_EQ (Di.bf.pose.p.y, solution.bf.pose.p.y);
    EXPECT_EQ (Di.bf.pose.q.GetAngle (), solution.bf.pose.q.GetAngle ());
...
TEST_F (ConfiguratorTestGetObstacle, AvoidNoGoal)
{
    ...
    BodyFeatures bf = bodyFeatures (.55, 0, 0, 0.02, 0.05);
    bf.attention = 1;
    transitionSystem[MOVING_VERTEX].Di
        = Disturbance (bf); //current task was avoiding
    ...
    Disturbance Di
        = getDisturbance (transitionSystem, MOVING_VERTEX, world, LEFT,
                          transitionSystem[MOVING_VERTEX].endPose);
    EXPECT_EQ (Di.bf.pose.p.x, solution.bf.pose.p.x);
    EXPECT_EQ (Di.bf.pose.p.y, solution.bf.pose.p.y);
    EXPECT_EQ (Di.bf.pose.q.GetAngle (), solution.bf.pose.q.GetAngle ());
    EXPECT_EQ (Di.bf.halfLength, solution.bf.halfLength);
...
TEST_F (ConfiguratorTest, GetDisturbance180Turn)
{
    ...
    Disturbance Di = getDisturbance (transitionSystem, v1, world, LEFT,
                                     transitionSystem[v1].endPose);
...
{
    Disturbance Di (PURSUE, b2Vec2 (0.81, 0.23)), Dn (AVOID, b2Vec2 (0.22, 0)),
        goal (PURSUE, b2Vec2 (1.0, 0));
...
TEST (GraphTools, ToTaskEnd)
{
    ...
    std::vector<vertexDescriptor>::iterator it = plan.begin (),
                                            result = plan.end ();
...
TEST_P (ConfiguratorTestTransitionMatrix, naive)
{
    ...
    {
        target = Disturbance (PURSUE, std::get<0> (GetParam ()).p,
                              std::get<0> (GetParam ()).q.GetAngle ());
        target.validate ();
    ...
    applyTransitionMatrix (e.m_target, std::get<1> (GetParam ()), false,
                           currentVertex, m_plan);
    int expected = expectedOptions (std::get<1> (GetParam ()),
                                    std::get<2> (GetParam ()), target);
...
TEST_P (ConfiguratorTestTransitionMatrix, InPlanNotVisited0)
{
    ...
    iteration = 2;
    applyTransitionMatrix (MOVING_VERTEX, currentTask.get_direction (), false,
                           MOVING_VERTEX, m_plan);
    EXPECT_EQ (transitionSystem[MOVING_VERTEX].options.size (), 1);
    EXPECT_EQ (transitionSystem[MOVING_VERTEX].options[0],
               currentTask.get_direction ());
}
...
TEST_P (ConfiguratorTestTransitionMatrix, InPlanVisited)
{
    ...
    transitionSystem[e.m_target].direction
        = transitionSystem[m_plan[0]].direction;
    setPhi (transitionSystem[e.m_target]);
    applyTransitionMatrix (MOVING_VERTEX,
                           transitionSystem[e.m_target].direction, false,
                           MOVING_VERTEX, m_plan);
    EXPECT_EQ (transitionSystem[MOVING_VERTEX].options, solution);
}

INSTANTIATE_TEST_CASE_P (
    SimulationOutcomes, ConfiguratorTestTransitionMatrix,
    ::testing::Values (
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (1.0, 0), b2Rot (0)), DEFAULT,
            simResult::successful),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (1.0, 0), b2Rot (0)), LEFT,
            simResult::successful),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (1.0, 0), b2Rot (0)), UNDEFINED,
            simResult::successful),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (1.0, 0), b2Rot (0)), DEFAULT,
            simResult::crashed),
        ...
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (1.0, 0), b2Rot (0)), DEFAULT,
            simResult::safeForNow),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (-1.0, 0), b2Rot (-M_PI)), RIGHT,
            simResult::successful),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform_inf, DEFAULT, simResult::crashed),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform_inf, DEFAULT, simResult::successful),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform_inf, DEFAULT, simResult::safeForNow),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (1.0, 0), b2Rot (0)), STOP,
            simResult::crashed),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (1.0, 0), b2Rot (0)), STOP,
            simResult::successful),
        ...
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (.78, .2), b2Rot (0)), DEFAULT,
            simResult::successful),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (.78, .2), b2Rot (0)), LEFT,
            simResult::successful),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (.78, .2), b2Rot (0)), UNDEFINED,
            simResult::successful),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (.78, .2), b2Rot (0)), DEFAULT,
            simResult::crashed),
        ...
        std::tuple<b2Transform, Direction, simResult::resultType> (
            ...
            simResult::safeForNow),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (.78, .2), b2Rot (-M_PI)), RIGHT,
            simResult::successful),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (.78, .2), b2Rot (0)), STOP,
            simResult::crashed),
        std::tuple<b2Transform, Direction, simResult::resultType> (
            b2Transform (b2Vec2 (.78, .2), b2Rot (0)), STOP,
            simResult::successful),
...
TEST_P (ConfiguratorTest2DT, adjustGoal)
{
    ...
    Disturbance obstacle (
        PURSUE, b2Vec2 (0.45, 0),
        0); //robot is driving towards an obstacle before it avoids it
    init (goal);
    ...
    update_graph (transitionSystem, tr);
    b2Transform expected
        = b2MulT (controller.get_disturbance ().pose (),
                  transitionSystem[plan_end ()]
                      .Di.pose ()); //position of goal wrt current disturbance
    /**/
    adjust_goal_expectation (); //what we're actually testing
    b2Transform observed = b2MulT (currentTask.get_disturbance ().pose (),
                                   controlGoal.get_disturbance ().pose ());
...
INSTANTIATE_TEST_CASE_P (
    AdjustGoal, ConfiguratorTest2DT,
...
TEST_P (ConfiguratorTest32DT, splitTask)
{
    ...
    vertexDescriptor v1
        = make_v1_crashed (currentVertex, std::get<0> (GetParam ()),
                           std::get<1> (GetParam ()),
                           std::get<2> (GetParam ()))
              .m_target;
    std::vector<vertexDescriptor> split
        = splitTask (v1, transitionSystem[v1].direction, currentVertex);
    b2Vec2 endPosition = std::get<1> (GetParam ()).p;
    int expected_splitSize = std::min (
        int (endPosition.Length () / (simulationStep + 0.00001)) + 2, 3);
    ...
    {
        float step_size
            = (transitionSystem[v].endPose.p - transitionSystem[v].start.p)
                  .Length ();
...
TEST_F (ConfiguratorTest, splitWithoutDummy)
{
    ...
    std::vector<vertexDescriptor> split
        = splitTask (v1, transitionSystem[v1].direction, v0);
    EXPECT_EQ (split.size (), 2);
}

INSTANTIATE_TEST_CASE_P (
    SplitTask, ConfiguratorTest32DT,
    ::testing::Values (
        std::tuple<b2Transform, b2Transform, b2Transform> (
            b2Transform_zero, b2Transform (b2Vec2 (0.6, 0), b2Rot (0)),
            b2Transform_zero),
        std::tuple<b2Transform, b2Transform, b2Transform> (
            b2Transform (b2Vec2 (0, 0), b2Rot (-M_PI_2)),
            b2Transform (b2Vec2 (0.26, -0.01), b2Rot (-M_PI_2)),
            b2Transform (b2Vec2 (0.265, -0.16), b2Rot (0))),
        std::tuple<b2Transform, b2Transform, b2Transform> (
            b2Transform (b2Vec2 (0, 0), b2Rot (M_PI_2)),
            b2Transform (b2Vec2 (0, 0.6), b2Rot (M_PI_2)), b2Transform_zero),
        std::tuple<b2Transform, b2Transform, b2Transform> (
            b2Transform (b2Vec2 (0.2, 0.27), b2Rot (0)),
            b2Transform (b2Vec2 (0.75, 0.27), b2Rot (0)), b2Transform_zero),
        std::tuple<b2Transform, b2Transform, b2Transform> (
            b2Transform (b2Vec2 (0.27, 0.2), b2Rot (M_PI_2)),
            b2Transform (b2Vec2 (.27, 0.75), b2Rot (M_PI_2)),
            b2Transform_zero),
        std::tuple<b2Transform, b2Transform, b2Transform> (
            b2Transform_zero, b2Transform (b2Vec2 (0.27, 0), b2Rot (0)),
            b2Transform_zero),
        std::tuple<b2Transform, b2Transform, b2Transform> (
            b2Transform_zero, b2Transform (b2Vec2 (0.18, 0), b2Rot (0)),
            b2Transform_zero)));
...
TEST_P (ConfiguratorBacktrackTest, Backtrack)
{
    ...
    std::vector<vertexDescriptor> evaluationQ = { v1, v2, v3 },
                                  all = evaluationQ, priorityQ;
    ...
    {
        EXPECT_TRUE (
            transitionSystem[v].visited ()); //check vertex marked as visited
    }
    EXPECT_NE (priorityQ[0],
               v1); //check that v1 is not the vertex with highest priority
}
...
TEST_P (ConfiguratorTakeBool, GetExploredTransitions)
{
    ...
    EXPECT_EQ (getExploredDirections (currentVertex, directions).size (),
               solution);
}
...
TEST_P (ConfiguratorTakeBool, FrontierVertices)
{
    ...
    {
        transitionSystem[*vi].phi = evaluationFunction (
            estimateCost (transitionSystem[*vi], b2Transform_zero,
                          transitionSystem[*vi].direction, controlGoal),
            *vi, m_plan);
    }
    std::vector<Frontier> frontiers
        = frontierVertices (MOVING_VERTEX, transitionSystem, info);
...
TEST_P (ConfiguratorTakeBool, PropagateDisturbance)
{
    ...
    if (GetParam ())
    {
        vertex_set_direction (currentVertex,
                              transitionSystem[e.m_target].direction);
        hasSameDn = false;
    }
    propagateD (e.m_target, currentVertex);
    EXPECT_EQ (vertex_get_Dn (e.m_target) == vertex_get_Dn (currentVertex),
               hasSameDn);
}
...
{
    transitionSystem = TransitionSystem (5);
    std::vector<vertexDescriptor> pq = { 2, 3, 4 }, plan = { 1, 2 },
                                  solution = { 2, 3, 1 };
...
TEST_P (ConfiguratorTakeBool, CheckVectorForPredicate)
{
    ...
    add_vertex_now (currentVertex, v1, controlGoal.get_disturbance (), e,
                    true);
...
TEST_P (ConfiguratorTakeBool, EvaluationFunctionSame)
{
    ...
    for (Direction d : directions)
    {
        ...
        else
        {
            ...
            transitionSystem[currentVertex].Dn.setPose (
                b2Transform (b2Vec2 (0.5, 0), b2Rot (0)));
        }
        EndedResult er = estimateCost (transitionSystem[currentVertex],
                                       b2Transform_zero, d, controlGoal);
        vertex_set_phi (currentVertex,
                        evaluationFunction (er, currentVertex, m_plan));
    }
...
INSTANTIATE_TEST_CASE_P (
    Backtrack, ConfiguratorBacktrackTest,
    ::testing::Values (std::tuple<b2Transform, b2Transform, b2Transform> (
                           b2Transform_zero,
                           b2Transform (b2Vec2 (0.6, 0), b2Rot (0)),
                           b2Transform_zero),
                       std::tuple<b2Transform, b2Transform, b2Transform> (
                           b2Transform (b2Vec2 (0, 0), b2Rot (-M_PI_2)),
                           b2Transform (b2Vec2 (0.26, -0.01), b2Rot (-M_PI_2)),
...
class ConfiguratorTaskVerticesTest
    : public ConfiguratorTest,
      public testing::WithParamInterface<std::tuple<bool, Direction> >
{
};

TEST_P (ConfiguratorTaskVerticesTest, SameTaskVertices)
{
    std::cout << "Testing if AttentiveConfigurator::task_vertices returns "
                 "vertices of all the same task"
              << std::endl;
    Direction direction = std::get<1> (GetParam ());
...
INSTANTIATE_TEST_CASE_P (Directions, ConfiguratorTaskVerticesTest,
...
{
    vertexDescriptor v0
        = make_successful (MOVING_VERTEX, std::get<0> (GetParam ())).m_target,
        v1;
...
TEST_P (ConfiguratorTakeBool, simulateModule)
{
    ...
    std::vector<vertexDescriptor> solution
        = { new_vertices[0], new_vertices[2], new_vertices[5] };
    ...
    if (!GetParam ())
    {
        ...
        eqm.addToEvaluationQueue (evaluationQ, new_vertices[5],
                                  transitionSystem, MOVING_VERTEX);
    }
    EXPECT_EQ (evaluationQ.size (), solutionSize);
...
INSTANTIATE_TEST_CASE_P (
    DirectionsAndOutcomes, ConfiguratorEvaluationQueueManagerTest,
    testing::Combine (testing::Values (LEFT, RIGHT),
                      testing::Values (simResult::successful,
                                       simResult::crashed)));
...
TEST_F (MovingVertexTest, DirectionIsDefault)
{
    ...
    EXPECT_TRUE (vertex_get_Di (MOVING_VERTEX)
...
TEST_P (ReactiveConfTest, Simulate)
{
    ...
    worldBuilder->makeBody (
        world, BodyFeatures (b2Transform (b2Vec2 (0.67, 0), b2Rot (0))));
...
INSTANTIATE_TEST_CASE_P (
    PlannedTasks, LogicalCheckPlanTest,
    ::testing::Values (
...
INSTANTIATE_TEST_CASE_P (
    PlannedTasksNotOk, LogicalCheckPlanTest,
    ::testing::Values (
        std::tuple<vertexDescriptor, AffordanceIndex, Direction, bool> (
            3, 1, DEFAULT, false),
        std::tuple<vertexDescriptor, AffordanceIndex, Direction, bool> (
...
INSTANTIATE_TEST_CASE_P (
    UnplannedTasks, LogicalCheckPlanTest,
    ::testing::Values (
        std::tuple<vertexDescriptor, AffordanceIndex, Direction, bool> (2, 0,
                                                                        LEFT,
                                                                        false),
        std::tuple<vertexDescriptor, AffordanceIndex, Direction, bool> (
            5, 0, DEFAULT, false)));
...
INSTANTIATE_TEST_CASE_P (Directions, ReactiveConfTest,
                         ::testing::Values (DEFAULT, RIGHT, LEFT));