...
class DataInterface
{
  ...
  public:
    ...
    void set_folder (std::string str) { folder = str; }

    int get_iteration () { return iteration; }

    void set_iteration (int i) { iteration = i; }

    void registerConfigurator (Configurator *i) { configurator = i; }

    bool has_interface () { return configurator != NULL; }

    void reset () { folder.clear (); }

    bool hasFolder () { return folder.size () > 0; }
